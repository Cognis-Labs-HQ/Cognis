...
import { createSystemRoutes } from '../src/routes/system-routes.js';
...
test('system route handles healthcheck endpoint', async () => {
  ...
  } as any, new URL('http://localhost/api/v1/system/healthcheck'));
...
test('system route exposes env-backed ui config', async () => {
  ...
  } as any, new URL('http://localhost/api/v1/system/ui-config'));
