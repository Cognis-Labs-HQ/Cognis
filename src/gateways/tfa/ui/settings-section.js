...
export function createSettingsSection({ i18n, root, markDirty }) {
            ...
            .map((entry, index) => {
                ...
                return `
                    <tr data-recovery-code-row="${escapeHtml(entry.id)}">
                      ...
                      <td class="settings-recovery-code-status-cell">${entry.used ? `<span class="settings-recovery-code-used-marker">\u2715 ${i18n.t("gateway.tfa.settings.recovery_codes_used")}</span>` : `<span class="settings-recovery-code-ready-marker">${i18n.t("gateway.tfa.settings.recovery_codes_ready_marker")}</span>`}</td>
                    </tr>
                `;
            })
            .join("");
    ...
    async function runTfaSetupFlow(methodId) {
        ...
        try {
            popupResult = await openPopup({
                title: i18n.t("gateway.tfa.settings.setup_title"),
                maxWidth: "520px",
                body: () => `
                <div class="stack">
    ...
    async function openConfiguredMethodPopup(methodId) {
        ...
        try {
            action = await openPopup({
                title: i18n.t("gateway.tfa.settings.method_manage_title"),
                maxWidth: "520px",
                body: () => `
                <div class="stack">
        ...
        try {
            while ((tfaStatus?.enabledMethods?.length ?? 0) === 0) {
                ...
                const action = await openPopup({
                    title: i18n.t("gateway.tfa.settings.required_title"),
                    body: () => `
                    <div class="stack">
                      <p>${escapeHtml(i18n.t("gateway.tfa.settings.required_prompt"))}</p>
    ...
    function renderBody() {
        ...
        if (!tfaStatus) {
            return `<p>${i18n.t("gateway.tfa.settings.loading")}</p>`;
        }
        const recoveryCodesHint = renderInfoTooltip(
            i18n.t("gateway.tfa.settings.recovery_codes_hint"),
    ...
    function renderContent() {
        return `<div id="tfa-settings-panel">${renderBody()}</div>`;
    }
    ...
    return {
        ...
        preferenceKey: "settings-tfa-layout",
        renderContent,
        async onRender() {
            await ensurePageStylesheet("/static/gateways/tfa/settings.css");
            [tfaStatus, recoveryCodesStatus] = await Promise.all([
                fetchTfaStatus(),
