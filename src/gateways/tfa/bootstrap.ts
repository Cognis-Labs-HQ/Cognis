...
export async function bootstrap(ctx: GatewayBootstrapContext): Promise<void> {
  ...
  ctx.gatewayRegistry.register({
    ...
    version: '1.0.0',
    description: 'Manages two-factor authentication methods and login checks.',
  ...
  ctx.capabilities.contribute(
    'tfa:resetUser',
    async (accountId: string) => gateway.resetUser(accountId),
  );
  ctx.capabilities.contribute(
    'tfa:getEnforceAllUsers',
    async () => gateway.getEnforceAllUsers(),
  );
  ctx.capabilities.contribute(
...
function createTfaRoutes(
  ...
  return async (
    ...
    if (setupBeginMatch && req.method === 'POST') {
      ...
      const displayName = String(body.displayName ?? claims.sub).trim() || claims.sub;
      try {
        ...
        res.end(
          JSON.stringify({
            error: {
              ...
              message: result.message || 'Setup verification failed',
            },
          }),
        );
        return true;
    ...
    if (url.pathname === '/api/v1/tfa/methods/preferences' && req.method === 'PUT') {
      const claims = requireAuth(req, res, 'user');
      if (!claims) return true;
      const body = await readJson(req);
      const preferredMethodIds = Array.isArray(body.methodIds)
        ? body.methodIds
            .filter((methodId: unknown) => typeof methodId === 'string')
    ...
    if (url.pathname === '/api/v1/tfa/recovery-codes/rotate' && req.method === 'POST') {
      const claims = requireAuth(req, res, 'user');
      if (!claims) return true;
    ...
    if (url.pathname === '/api/v1/tfa/recovery-codes/status' && req.method === 'GET') {
      const claims = requireAuth(req, res, 'user');
      if (!claims) return true;
    ...
    if (url.pathname === '/api/v1/tfa/enforcement' && req.method === 'GET') {
      const claims = requireAuth(req, res, 'admin');
      if (!claims) return true;
    ...
    if (url.pathname === '/api/v1/tfa/enforcement' && req.method === 'PUT') {
      const claims = requireAuth(req, res, 'admin');
      if (!claims) return true;
      const body = await readJson(req);
...
function createTfaAdapterAdminRoutes(
  ...
  return async (
    ...
    const stateMatch = url.pathname.match(new RegExp(`^${base}/([^/]+)/(enable|disable)$`));
