...
export interface TfaMethodAdapter {
  ...
  beginSetup(input: {
    ...
    displayName: string;
    issuer: string;
  }): Promise<{ pendingPayload: Record<string, unknown>; view: TfaMethodSetupView }>;
  verifySetup(input: {
    accountId: string;
    pendingPayload: Record<string, unknown>;
    verification: Record<string, unknown>;
  }): Promise<{ verified: boolean; state?: Record<string, unknown>; message?: string }>;
  verifyLogin(input: {
    accountId: string;
...
export class CoreTfaGateway {
  ...
  async saveAdapterConfig(adapterId: string, config: Record<string, unknown>): Promise<void> {
  ...
  async getUserStatus(accountId: string): Promise<UserTfaStatus> {
    ...
    const enabledMethods = userMethods
      ...
      .map((method) => ({
        id: method.methodId,
        name: this.adapters.get(method.methodId)?.name ?? method.methodId,
        enabled: true,
    ...
    const availableMethods = Array.from(this.adapters.values())
      ...
      .map((adapter) => ({
        ...
        configuredAt:
          userMethods.find((method) => method.methodId === adapter.id)?.configuredAt ??
          null,
      }));

    const hasRecoveryCodes = await this.store.hasUnusedRecoveryCodes(accountId);
  ...
  async beginMethodSetup(input: {
    ...
    displayName: string;
    methodId: string;
  }): Promise<{ setupId: string; methodId: string; methodName: string; view: TfaMethodSetupView }> {
  ...
  async verifyMethodSetup(input: {
    ...
    const nextSortOrder = await this.store.getNextSortOrder(input.accountId);
  ...
  async setPreferredMethods(accountId: string, methodIds: string[]): Promise<void> {
    await this.store.setPreferredOrder(accountId, methodIds);
  }

  async generateRecoveryCodes(accountId: string, count = 8): Promise<string[]> {
  ...
  async verifyLogin(accountId: string, methodId: string, payload: Record<string, unknown>): Promise<{ verified: boolean; message?: string }> {
    if (methodId === 'recovery_code') {
      const recoveryCode = String(payload.code ?? '').trim();
      if (!recoveryCode) {
        return { verified: false, message: 'recovery_code_required' };
      }
      const valid = await this.store.consumeRecoveryCode(accountId, recoveryCode);
      return valid
        ? { verified: true }
  ...
  async getLoginMethods(accountId: string): Promise<Array<{ id: string; name: string }>> {
  ...
  async discoverAdapters(tfaAdaptersRoot: string): Promise<void> {
    ...
    for (const entry of entries) {
      const packagePath = path.join(tfaAdaptersRoot, entry, 'package.json');
      try {
        ...
        const entryPath = path.resolve(tfaAdaptersRoot, entry, packageJson.main);
