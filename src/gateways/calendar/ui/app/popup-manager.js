...
export function createCalendarPopupManager({
    ...
    async function handleEventResponse(eventData, responseOption) {
        ...
        if (!response.ok) {
            showToast(i18n.t("gateway.calendar.response_update_failed"), "error");
            return false;
        }
        if (targetCalendarId) {
            setSelectedCalendarId(targetCalendarId);
        ...
        await reloadState();
        refreshComposer();
        showToast(i18n.t("gateway.calendar.response_update_success"), "success");
        return true;
    }

    async function respondToEventSelection(calendarId, eventId, responseOption) {
        try {
