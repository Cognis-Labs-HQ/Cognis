...
Calendar は Share ページ契約で `preserveShareShell` を要求します。レンダラーは分離されたカレンダーコンポーネントだけを Share のリソーススロットへマウントするため、標準共有ヘッダーとページの基本構成が維持されます。フォーム、ポップアップ、時刻、カレンダー表示の依存関係を直接 import し、渡された限定ゲストトークンで予定変更を認証します。
