...
- `GET /api/v1/logging/stream` für die Seite Administration → Protokolle bereitstellen (admin-only SSE-Stream mit Schweregrad-/Stichwortfiltern und `LOG_LEVEL`-Basisfilter).
...
Standardmäßig schreibt der Logger Konsolenausgaben und speichert weiterhin JSON-Zeilen in der persistenten Protokolldatei. `LOG_LEVEL` wirkt als Basisfilter für den Admin-Log-Stream; die Datei enthält weiterhin alle Log-Level.
