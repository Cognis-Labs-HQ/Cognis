...
デフォルトでは、Logger はコンソール出力を書き出し、永続ログファイルには引き続き JSON 行を保存します。`LOG_LEVEL` は stdout/stderr をフィルターし、`LOG_FILE_LEVEL` は永続ファイルへの書き込みを個別にフィルターします。管理者は実行時に各アダプター設定を上書きし、環境値へリセットできます。
