...
function createStudyAdapterRoutes(
  ...
  return async (
    ...
    if (modulesMatch && req.method === 'GET') {
      ...
      const components = gateway.listChildComponents(languageCode, claims.role);
      res.writeHead(200, { 'content-type': 'application/json' });
...
export async function bootstrap(ctx: GatewayBootstrapContext): Promise<void> {
  ...
  const syncModuleEnabledState = (moduleId: string, enabled: boolean): void => {
   ...
   */
  ctx.capabilities.contribute('modules:onStateChanged', syncModuleEnabledState);

  /**
   * Resolves a language code to module metadata, then evaluates enablement
   * through the module-state-aware checker.
   */
  const isLanguageEnabled = async (languageCode: string): Promise<boolean> => {
  ...
  ctx.uiRegistry?.registerNavbarPlugin({
    ...
    isEnabled: () => ctx.gatewayRegistry.get('study')?.status !== 'disabled',
  });
