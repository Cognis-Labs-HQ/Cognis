...
データベースゲートウェイは、Cognisにおけるすべてのデータベース操作への単一のアクセスポイントです。PostgreSQLとMariaDBの違いを隠す統一されたエグゼキューターインターフェースを提供します。ゲートウェイは環境から `DB_TYPE` を読み取り、適切なエグゼキューターを作成し、スキーマを初期化し、エグゼキューターとダイアレクトヘルパーをケイパビリティストアに提供します。
