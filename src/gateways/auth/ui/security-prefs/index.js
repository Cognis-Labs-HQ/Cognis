...
export function createSettingsSection({ i18n, root }) {
  ...
  async function loadCapability() {
    const response = await apiFetch('/api/v1/auth/password-change-capability');
    if (!response.ok) {
      const payload = await response.json().catch(() => null);
      capability = {
        supported: false,
        reason:
          payload?.error?.message || i18n.t('gateway.auth.security.load_failed'),
      };
      return;
    }
  ...
  function bindPasswordResetButton() {
    const button = settingsRoot.querySelector('#settings-reset-password-btn');
  ...
  return {
    ...
    async onRender() {
      ...
      if (
        ...
        showToast(
          capability.reason || i18n.t('gateway.auth.security.unsupported_default'),
          {
            variant: 'warning',
          },
        );
