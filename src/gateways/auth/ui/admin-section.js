...
export function createAdminSection({
    ...
    function renderProviders() {
        ...
        return adapters
            .map((adapter) => {
                ...
                const hasConfig = Array.isArray(adapter.schema) && adapter.schema.length > 0;
    ...
    function renderContent() {
        ...
        return template
            ...
            .replaceAll('{{policyHeading}}', escapeHtml(i18n.t('gateway.auth.policy_heading')))
            .replaceAll('{{minLengthLabel}}', escapeHtml(i18n.t('gateway.auth.policy_min_length')))
            .replaceAll('{{minLengthValue}}', escapeHtml(String(passwordPolicy.minLength)))
            .replaceAll('{{uppercaseLabel}}', escapeHtml(i18n.t('gateway.auth.policy_require_uppercase')))
            .replaceAll('{{uppercaseValue}}', escapeHtml(String(passwordPolicy.requireUppercase)))
            .replaceAll('{{lowercaseLabel}}', escapeHtml(i18n.t('gateway.auth.policy_require_lowercase')))
            .replaceAll('{{lowercaseChecked}}', passwordPolicy.requireLowercase ? ' checked' : '')
            .replaceAll('{{digitLabel}}', escapeHtml(i18n.t('gateway.auth.policy_require_digit')))
            .replaceAll('{{digitValue}}', escapeHtml(String(passwordPolicy.requireDigit)))
            .replaceAll('{{specialLabel}}', escapeHtml(i18n.t('gateway.auth.policy_require_special')))
            .replaceAll('{{specialValue}}', escapeHtml(String(passwordPolicy.requireSpecial)))
            .replaceAll('{{providersHeading}}', escapeHtml(i18n.t('ui.reuse.adapters')))
    ...
    function bindPasswordPolicy(root) {
        ...
        saveButton.addEventListener('click', async () => {
            const minLengthInput = root.querySelector('#auth-policy-min-length');
            const uppercaseInput = root.querySelector('#auth-policy-require-uppercase');
            const lowercaseInput = root.querySelector('#auth-policy-require-lowercase');
            const digitInput = root.querySelector('#auth-policy-require-digit');
            const specialInput = root.querySelector('#auth-policy-require-special');
            const updatedPolicy = {
                minLength:
                    minLengthInput instanceof HTMLInputElement
                        ? parsePolicyMinLength(minLengthInput.value, passwordPolicy.minLength)
                        : passwordPolicy.minLength,
                requireUppercase:
                    uppercaseInput instanceof HTMLInputElement
                        ? parsePolicyCount(uppercaseInput.value, passwordPolicy.requireUppercase)
                ...
                requireDigit:
                    digitInput instanceof HTMLInputElement
                        ? parsePolicyCount(digitInput.value, passwordPolicy.requireDigit)
                        : passwordPolicy.requireDigit,
                requireSpecial:
                    specialInput instanceof HTMLInputElement
                        ? parsePolicyCount(specialInput.value, passwordPolicy.requireSpecial)
