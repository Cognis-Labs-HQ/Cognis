...
Sitzungen externer Anbieter durchlaufen `gateAccountCreation` vor `ensureExternalAccount`. Wenn die offene Registrierung deaktiviert ist, muss die Sitzung ein Registrierungstoken und eine passende Anbieter-E-Mail enthalten. Eine angehaltene Sitzung gibt `account_creation_required` mit `emailRequired`, `registrationTokenRequired` und `retryEndpoint` zurück. Die Anbieteroberfläche sendet das Token und eine gegebenenfalls angeforderte E-Mail mit derselben Anbieter-ID an diesen Endpunkt; Cognis übernimmt diese Werte in die Freigabe der Kontoerstellung, auch wenn der Anbieteradapter nur seine authentifizierte Identität zurückgibt. Ein Abbruch statt eines erneuten Versuchs beendet die Anmeldung, ohne ein Konto anzulegen.
