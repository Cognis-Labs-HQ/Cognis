...
- Berkontribusi `auth:accountStore`, `auth:createLocalAdmin`, `auth:getLoginMethods`, dan `auth:registerPageScriptOrigin` ke capability store.
...
| `auth:registerPageScriptOrigin` | `(origin) => string \| null`                   | Mendaftarkan origin skrip http(s) tepercaya untuk header CSP halaman |
