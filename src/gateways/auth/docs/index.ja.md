...
SSO モジュールは CTX を介して `auth:registerExternalProfileProvider` を登録できます。リゾルバーはプロバイダー ID、Cognis アカウント ID、外部ユーザー ID、認証済みプロバイダーセッションを受け取り、表示名、自己紹介、所在地、Web サイト、アバターおよびバナーのデータを返せます。プロフィールアダプターが外部アカウントの初回作成時に自身の保存機能で反映します。
