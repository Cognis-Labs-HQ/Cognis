...
export function formatStructured(value: unknown): string {
  const normalized = typeof value === 'string' ? normalizeResponse(value) : value
...
function formatTable(
  ...
  const widths = columns.map(({ key, label }) =>
    rows.reduce(
      (width, row) =>
        Math.max(width, String(row[key] ?? FIELD_EMPTY_PLACEHOLDER).length),
      label.length,
    ),
  )
  ...
  const body = rows.map((row) =>
    columns
      .map(({ key }, index) =>
        String(row[key] ?? FIELD_EMPTY_PLACEHOLDER).padEnd(widths[index]),
      )
      .join('  '),
  )
...
export function renderUserMutation(
  ...
  const normalizedPayload = normalizeResponse(payload) as Record<string, unknown>
