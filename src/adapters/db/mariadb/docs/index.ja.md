...
宣言されたタイムスタンプ列に渡されたISO 8601値はMariaDBの `DATETIME` 入力構文へ変換し、テキスト列に保存するISO形式の値は変更しません。初期化用の生SQLで作成されたテーブルへのコマンドも、サーバーエラーに完全修飾列名が含まれる場合を含め、標準的なMariaDB日時パラメーターで一度再試行して復旧します。
