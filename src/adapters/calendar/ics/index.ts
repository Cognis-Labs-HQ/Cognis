...
function createIcsRoutes(ctx: CalendarAdapterBootstrapCtx) {
    ...
    return async (req, res, url): Promise<boolean> => {
        if (url.pathname === '/api/v1/calendar/ics/export' && req.method === 'GET') {
            const claims = routeContext.requireAuth(req, res, 'user');
            if (!claims) return true;
            const calendarId = String(url.searchParams.get('calendarId') ?? '').trim();
        ...
        if (url.pathname === '/api/v1/calendar/ics/import' && req.method === 'POST') {
            const claims = routeContext.requireAuth(req, res, 'user');
            if (!claims) return true;
