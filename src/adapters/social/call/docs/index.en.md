...
Moving an active component to picture-in-picture uses the browser's state-preserving DOM move when available, so embedded conference media and connections continue without remounting. Component removal is observed from the moment mounting succeeds—not only after entering picture-in-picture—so a provider-driven meeting end immediately removes the call surface and restores the conversation.
