...
Call history uses the generic `social:messages:appendRoomEvent` capability and contributes its localized event formatter to the `messages:format-room-event` flow. Call event names, details, translations, and presentation therefore remain owned by the Call adapter rather than the Messages store or renderer.
