...
Riwayat panggilan menggunakan capability generik `social:messages:appendRoomEvent` dan menyumbangkan pemformat peristiwa terlokalisasinya ke flow `messages:format-room-event`. Nama, detail, terjemahan, dan tampilan peristiwa panggilan tetap dimiliki adapter Call, bukan store atau renderer Messages.
