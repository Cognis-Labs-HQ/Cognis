  ...
  const elements = [
    {
      ...
      onRender: () => {
        ...
        const composerInput = document.getElementById('messages-composer-input');
        const composerSendButton = form?.querySelector('.messages-composer-send');
        const composerPreview = document.getElementById('messages-composer-preview');
        ...
        const renderComposerPreview = () => {
          ...
          const contentValue =
            composerInput instanceof HTMLTextAreaElement ? composerInput.value : '';
        ...
        threadList?.addEventListener(
          ...
          async (clickEvent) => {
            hideAllMessageHoverPopups();
            const moreButton = clickEvent.target.closest('[data-reaction-more]');
            if (moreButton instanceof HTMLElement) {
              const messageId = moreButton.getAttribute('data-message-id');
              if (messageId) await roomState.openEmojiPickerPopup(messageId);
              return;
            }
            ...
            if (reactionDetailsButton instanceof HTMLElement) {
              ...
              const parsedDetails = parseEncodedPayload(rawDetailsPayload);
            ...
            if (!selectedRoomId || !(threadList instanceof HTMLElement)) return;
            const beforeTime = loadEarlierButton.getAttribute('data-before-time');
        ...
        threadList?.addEventListener(
          ...
          (mouseEvent) => {
            ...
            const rawReaders = statusElement.getAttribute('data-readers') ?? '[]';
        ...
        form?.addEventListener('submit', async (event) => {
          ...
          if (currentRoom?.canSend === false || currentRoom?.isArchived) {
            showToast(i18n.t('module.social.messages.archived_cannot_send'), {
              variant: 'error',
            });
            return;
          }
          ...
          try {
            ...
            console.error('[messages] requireRoomKey failed', error);
            key = null;
          }
          if (!key) {
            showToast(i18n.t('module.social.messages.key_unavailable'), {
              variant: 'error',
            });
            return;
          }
          ...
          if (!response.ok) {
            ...
            if (code === 'not_member') {
              showToast(i18n.t('module.social.messages.not_member_cannot_send'), {
                variant: 'error',
              });
  ...
  function bindSidebarEvents() {
    ...
    roomsList?.addEventListener('click', async (clickEvent) => {
      ...
      roomsList
        ...
        .forEach((activeItem) => activeItem.classList.remove('messages-room--active'));
    ...
    const templatesButton = document.getElementById('messages-open-templates-btn');
