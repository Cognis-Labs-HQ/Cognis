...
export function createMessageTemplatesUi({
  ...
  function renderSidebarTemplateList() {
    const listElement = document.getElementById('messages-sidebar-template-list');
  ...
  function renderTemplateEditorPreview() {
    ...
    const bodyValue =
      templateBodyInput instanceof HTMLTextAreaElement ? templateBodyInput.value : '';
  ...
  function bindTemplatePopupEvents(overlay) {
    const templateEditor = overlay.querySelector('#messages-template-editor');
    ...
    templateEditor?.addEventListener('submit', (submitEvent) => {
      ...
      if (existingIndex >= 0) {
        ...
        savedMessageTemplates = [templateRecord, ...savedMessageTemplates];
      }
    ...
    overlay.addEventListener('click', (clickEvent) => {
      const tokenButton = clickEvent.target.closest('[data-template-token]');
      if (!(tokenButton instanceof HTMLButtonElement)) return;
      const token = String(tokenButton.dataset.templateToken ?? '').trim();
      if (!token) return;
      if (!(templateBodyInput instanceof HTMLTextAreaElement)) {
        return;
      }
  ...
  async function handleSidebarTemplateClick(clickEvent) {
    const actionButton = clickEvent.target.closest('[data-template-action]');
    ...
    const deleteResult = await openPopup({
      title: i18n.t('module.social.messages.template_delete_confirm_title'),
