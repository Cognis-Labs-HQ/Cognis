...
import { fetchEmojiUsage, loadAllEmojis, recordEmojiUsage } from './emoji-helpers.js';
import {
...
import { encryptMessage, normalizeReactionEmoji } from './message-utils.js';
import { roomListRenderSignature, renderRoomList } from './room-render.js';
...
export function createMessagesRoomState({
  ...
  function syncComposerAvailability(room) {
    ...
    const canSend =
      Boolean(room) && room?.canSend !== false && room?.isArchived !== true;
    if (input) {
  ...
  async function openRoom(roomId) {
    ...
    if (room) {
      rooms = rooms.map((entry) =>
        String(entry.id) === String(room.id) ? { ...entry, ...room } : entry,
      );
    }
    if (headerSlot && room) {
      headerSlot.innerHTML = renderThreadHeader(room, currentAccountId, i18n);
  ...
  async function respondToPendingRequest(
    ...
    if (action === 'approve') {
      ...
      const nextRoomId = payload?.data?.id || roomIdHint || selectedRoomId;
      if (nextRoomId) {
        selectedRoomId = nextRoomId;
        history.pushState({}, '', `/messages/${encodeURIComponent(nextRoomId)}`);
  ...
  function bindPendingRequestBannerEvents() {
    ...
    banner
      ...
      ?.addEventListener('click', async () => {
        await respondToPendingRequest(requestId, 'approve', selectedRoomId);
      });
    banner
      .querySelector('.messages-request-banner-reject')
      ?.addEventListener('click', async () => {
        await respondToPendingRequest(requestId, 'reject', selectedRoomId);
      });
  }
    ...
    await openPopup({
      ...
      onOpen: (overlay) => {
        ...
        grid?.addEventListener('click', async (event) => {
          const button = event.target.closest('.messages-emoji-picker-btn');
          if (!(button instanceof HTMLButtonElement)) return;
          const chosenEmoji = button.dataset.emoji;
          overlay.querySelector('[data-popup-action]')?.click();
          recordEmojiUsage(apiFetch, chosenEmoji, normalizeReactionEmoji);
          await toggleReaction(messageId, chosenEmoji);
        });

        searchInput?.addEventListener('input', () => {
          const query = searchInput.value.normalize('NFC').toLowerCase().trim();
  ...
  async function refreshTypingIndicator() {
    ...
    const typingStatusElement = document.getElementById('messages-typing-status');
    ...
    memberSummaryButton?.addEventListener('click', async () => {
      ...
      await openPopup({
        ...
        body: renderMemberSummaryBody({
          ...
          emptyText: i18n.t('module.social.messages.member_summary_empty'),
        }),
        onOpen: (overlay) => {
          overlay.addEventListener('error', handleProfileAvatarError, {
            capture: true,
          });
  ...
  async function leaveSelectedRoom(handle) {
    ...
    if (!rooms.length) {
      ...
      const headerSlot = document.getElementById('messages-thread-header-slot');
      const bannerSlot = document.getElementById('messages-request-banner-slot');
      const typingStatus = document.getElementById('messages-typing-status');
      const threadList = document.getElementById('messages-thread-list');
    ...
    const fallbackRoomId =
      rooms.find((room) => String(room.id) !== String(roomIdToLeave))?.id ??
      rooms[0].id;
  ...
  async function warmEmojiCaches() {
