...
function renderMessageStatus(
  ...
  if (readersHere.length > 0) {
    ...
    const avatarMarkup = readersHere
      .map((reader) => {
        const label = reader.displayName || reader.handle || reader.accountId;
...
function renderReactionRows(message, i18n, isOwn = false) {
  ...
  const reactionRows = Array.isArray(message.reactions) ? message.reactions : [];
      ...
      allReactions.map((reaction) => ({
        ...
        reactedBy: reaction.reactedBy
          .map((reactor) => {
            ...
            const resolvedLabel = resolveMemberDisplayName(normalizedReactor);
...
function renderMessageBodyMarkup(messageText) {
  const normalizedText = String(messageText ?? MESSAGE_UNAVAILABLE_PLACEHOLDER);
...
export async function openReactionDetailsPopup(reactionDetailsRows, i18n) {
  ...
  const detailRows = rows.flatMap((row) => {
    ...
    const reactedByRows = Array.isArray(row?.reactedBy) ? row.reactedBy : [];
    ...
    return reactedByRows
      .map((reactor) => {
        ...
        if (reactedAt) {
          ...
          reactedAtLabel = [reactedDay, reactedTime].filter(Boolean).join(' ');
        }
...
export async function loadRooms(i18n, { getRoomKey }) {
  ...
  return Promise.all(
    rooms.map(async (room) => {
      ...
      if (
        lastMessage.contentType === 'application/vnd.cognis.room-event+json'
      ) {
        return {
          ...room,
          lastMessagePreview: i18n.t('module.social.messages.preview_event'),
        };
      }
