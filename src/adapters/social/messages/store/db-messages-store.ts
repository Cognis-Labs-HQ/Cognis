...
export class DbMessagesStore {
  ...
  async findDmBetween(accountA: string, accountB: string): Promise<RoomRow | null> {
  ...
  async getPendingIncomingRoomMessageRequest(
    ...
    return getPendingIncomingRoomMessageRequest(this.db, roomId, toAccountId);
  }
  ...
  async approvePendingRequestsBetween(
    ...
    await approvePendingRequestsBetween(this.db, accountA, accountB, roomId);
  }
