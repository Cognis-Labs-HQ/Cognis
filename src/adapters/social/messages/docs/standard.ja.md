...
メッセージページは `Social Messages` キーリングスコープを通じてルームキーを解決します。ページを開くと既存のチャット秘密情報を読み込むためのアクセスを要求し、ロック解除をキャンセルしても配信キーを保存するための二度目の要求には進みません。ロック解除に成功した場合や再読み込み後に復元した場合は、ルーム一覧のプレビューとアクティブな会話の両方を更新し、暗号化済みプレースホルダーを直ちに置き換えます。
