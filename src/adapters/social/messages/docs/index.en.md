...
The adapter exposes the generic `social:messages:appendRoomEvent` capability for components that need to record a room-scoped timeline event. Its payload stores only neutral event metadata; owning components retain their domain-specific event names and fields. The `messages:format-room-event` browser flow lets those components provide localized presentation without adding their strings or rendering rules to Messages.
