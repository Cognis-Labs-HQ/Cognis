...
着信はスレッドヘッダーの直下に提供アクションバーとして表示され、左側にラベル、右側にプロバイダー所有の応答・拒否 SVG アクションを配置します。過去の通話イベントは操作プロンプトではなく通常のタイムライン記録として残ります。

このアダプターは、ルーム単位のタイムラインイベントを記録するコンポーネント向けに、汎用 Capability `social:messages:appendRoomEvent` を公開します。ペイロードには中立的なイベントメタデータだけを保存し、ドメイン固有のイベント名とフィールドは所有コンポーネントが管理します。ブラウザ Flow `messages:format-room-event` により、各コンポーネントは Messages に文字列や描画規則を追加せず、ローカライズ済みの表示を提供できます。
