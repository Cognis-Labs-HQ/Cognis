...
import { SmtpNotificationSender, SmtpTemporaryError, createNotificationSender } from '../smtp-notification-sender.js';
...
test('SmtpNotificationSender retries after greylisting (4xx on MAIL FROM) and succeeds on second attempt', async () => {
  ...
  try {
    const sender = new SmtpNotificationSender(
      { host: server.host, port: server.port, from: 'test@example.com', secure: 'none', greylistRetries: 1 },
      undefined,
      trackingSleep,
    );
    await sender.send({ category: 'test', recipientUsername: 'alice', recipientEmail: 'alice@example.com', subject: 'Hi', body: 'Test' });
...
test('SmtpNotificationSender does not retry on permanent SMTP errors (5xx)', async () => {
  ...
  try {
    const sender = new SmtpNotificationSender(
      { host: server.host, port: server.port, from: 'test@example.com', secure: 'none', greylistRetries: 2 },
      undefined,
      noopSleep,
    );
    await assert.rejects(
      () => sender.send({ category: 'test', recipientUsername: 'alice', recipientEmail: 'alice@example.com', subject: 'Hi', body: 'Test' }),
