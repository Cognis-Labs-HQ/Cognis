    ...
    const rows = layerEntries
        .map(
            (
                ...
                ${entry.editable === false || layer.semanticRole === "particle" ? "" : `<button class="library-admin-edit btn-neutral" type="button" data-library-admin-edit="${escapeHtml(entry.id)}" aria-label="${escapeHtml(i18n.t("gateway.study.library_admin_edit").replace("{{ entry }}", entry.label))}"><span aria-hidden="true"></span></button>`}
            </li>`,
        )
        .join("");
