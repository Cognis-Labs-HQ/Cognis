...
Every `cardConstructor` payload exposes `input_carousels` and `pronunciation_carousels`. Each array contains relationship IDs from that constructor: input carousels compose the primary value, while pronunciation carousels satisfy reading relationships. For alternate characters, only pronunciation-carousel minimums are required; unresolved primary references do not block creation. Lookup providers may omit provider-owned provenance and ranking metadata because the Library normalizes those values from the registered provider.
