...
Existing compound-writing-unit cards use the same provider-labelled pronunciation carousel as creation. The editor derives pronunciation from ordered atomic-writing-unit selections instead of showing a separate tag-entry field. It sorts selected references by authored position, suppresses duplicate target-layer carousels, and limits compound-writing-unit editing to the atomic layer that provides pronunciation. Editing is selection-only and does not expose a misleading nested-create action.
