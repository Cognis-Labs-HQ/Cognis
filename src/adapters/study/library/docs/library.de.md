...
Ebenen mit der Rolle `atomicWritingUnit` oder `compoundWritingUnit` deklarieren die erforderlichen Standardfelder `pronunciation` (`stringList`) und `audio` (`audio`). Inhaltspakete können MP3-, Ogg-, WAV-, WebM- oder M4A-Dateien oder eine HTTPS-URL bereitstellen. Lokale Dateien bleiben authentifizierte Paketressourcen. Entferntes Audio wird über die eintragsautorisierte Bibliotheksroute geladen, auf öffentliche HTTPS-Hosts und unterstützte Audio-Medientypen beschränkt, auf 10 MiB begrenzt und einmal je URL unter `COGNIS_LIBRARY_AUDIO_CACHE_DIR` (oder `.cognis-data/study-library-audio`) zwischengespeichert. Die gemeinsame serverseitige Datei vermeidet wiederholte Upstream-Downloads, während jede Anfrage weiterhin den Bereich des Eintrags prüft.
