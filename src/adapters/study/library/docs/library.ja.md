...
意味役割が `definition` の各レイヤーは、モジュール所有の文字列キープレフィックスと、文字列キーおよびローカライズ済みテキストのフィールドを宣言します。エントリー作成では英語を必須とし、Cognis が公開するすべての UI 言語を入力欄に表示して、生成したキーを定義レコードへ保存します。未入力言語は任意の `localization:translateString` ケイパビリティで補完できます。
