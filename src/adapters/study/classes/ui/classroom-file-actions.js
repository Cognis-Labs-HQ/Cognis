...
async function listNotepadFiles(apiFetch, classId) {
    const response = await apiFetch(
        `/api/v1/study/classes/${encodeURIComponent(classId)}/notes/files`,
    ).catch(() => null);
...
async function saveNotepadFile(apiFetch, classId, filename, content) {
    const response = await apiFetch(
        `/api/v1/study/classes/${encodeURIComponent(classId)}/notes/files/${encodeURIComponent(filename)}`,
        {
            method: "PUT",
...
async function renameNotepadFile(apiFetch, classId, oldName, newName) {
    const response = await apiFetch(
        `/api/v1/study/classes/${encodeURIComponent(classId)}/notes/files/rename`,
        {
            method: "POST",
