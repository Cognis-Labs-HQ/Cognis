...
interface ClassroomResources {
    ...
    agendaSnapshots: AgendaSnapshot[];
    updatedAt: string;
    updatedBy: string;
}
...
const DEFAULT_MAX_NOTE_BYTES = 256 * 1024;

function resolveMaxNoteBytes(getMaxBytes?: () => number): number {
    const candidate = Number(getMaxBytes?.() ?? DEFAULT_MAX_NOTE_BYTES);
    if (!Number.isFinite(candidate) || candidate <= 0) {
        return DEFAULT_MAX_NOTE_BYTES;
    }
...
export async function handleClassroomNotesRoutes(
    req: IncomingMessage,
    res: ServerResponse,
    url: URL,
    ctx: RouteContext,
    ...
    if (!resourcesCapability) return false;

    const agendaMatch = url.pathname.match(
        /^\/api\/v1\/study\/classes\/([^/]+)\/notes\/agenda$/,
    );
    ...
    if (agendaMatch && req.method === "PUT") {
        ...
        const maxBytes = ctx.getCapability<() => number>(
            "file-reader:text:maxFileBytes",
        );
        if (
            Buffer.byteLength(nextDocument, "utf8") >
            resolveMaxNoteBytes(maxBytes)
        ) {
            jsonError(
                res,
                413,
    ...
    const snapshotCreateMatch = url.pathname.match(
        /^\/api\/v1\/study\/classes\/([^/]+)\/notes\/agenda\/snapshots$/,
    );
    ...
    const openSnapshotMatch = url.pathname.match(
        /^\/api\/v1\/study\/classes\/([^/]+)\/notes\/agenda\/open$/,
    );
    ...
    const notepadFilesMatch = url.pathname.match(
        /^\/api\/v1\/study\/classes\/([^/]+)\/notes\/files$/,
    );
    if (notepadFilesMatch && req.method === "GET") {
        ...
        const fileGateway =
            ctx.getCapability<FileStorageGateway>("file:gateway") ?? null;
        if (!fileGateway) {
            jsonError(
                res,
                503,
    ...
    if (notepadFileMatch) {
        ...
        if (req.method === "PUT") {
            ...
            if (content.byteLength > resolveMaxNoteBytes(maxBytes)) {
                jsonError(
                    res,
                    413,
                    "payload_too_large",
    ...
    const notepadRenameMatch = url.pathname.match(
        /^\/api\/v1\/study\/classes\/([^/]+)\/notes\/files\/rename$/,
    );
