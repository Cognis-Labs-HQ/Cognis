...
ユーザー検証を完了すると、サーバーが保留中の設定に追加され、有効化をすぐに選択できます。この時点で有効化すると、保留中のサーバー一覧を保存してからアダプターを有効にします。ユーザー検証フォームで Enter キーを押すと認証テストが実行されます。未保存のサーバー設定を開始した後にセットアップを閉じる場合は、破棄の確認が必要です。

ユーザー認証を手動でテストする前に「設定を保存」を選択すると、同じ認証テストが自動的に実行されます。認証に失敗した場合、バインド項目を修正できるようセットアップは接続手順に戻ります。
