...
`DELETE /api/v1/auth/keyring` adalah kontrak reset destruktif. Sebelum menghapus vault buram, Authentication memanggil setiap pemilik yang terdaftar melalui `auth:registerKeyringDataOwner`, hanya menghapus akun dari ruang terenkripsi dan objek lain yang aksesnya bergantung pada entri keyring. Instans akun, profil, hubungan pengikut, dan data akun yang tidak bergantung tetap tidak berubah. Tindakan Settings saat terkunci memakai rute ini dan langsung memulai penyiapan keyring pertama kali. Saat terbuka, tindakan tetap hanya menghapus entri dari vault saat ini.
