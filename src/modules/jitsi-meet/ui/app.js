...
const SESSION_STORAGE_KEY = 'jitsi-meet:session-id';
  ...
  return `
    <section class="jitsi-page-layout">
        ...
        <div class="jitsi-stage-frame-wrap">
          ...
          <div id="jitsi-overlay" class="jitsi-overlay">
            ...
            <div class="jitsi-overlay-actions">
              <button id="jitsi-start-btn" class="btn btn-animated" type="button" disabled>${escapeHtml(i18n.t('module.jitsi_meet.overlay.start_meeting'))}</button>
              <button id="jitsi-auth-btn" class="btn btn-secondary" type="button" hidden>${escapeHtml(i18n.t('module.jitsi_meet.overlay.auth_required'))}</button>
              <button id="jitsi-reclaim-btn" class="btn btn-secondary" type="button" hidden>${escapeHtml(i18n.t('module.jitsi_meet.overlay.reclaim'))}</button>
            </div>
      ...
      <aside class="jitsi-chat-pane card-elevated">
        ...
        <a id="jitsi-chat-link" class="btn btn-secondary" href="/messages" hidden>${escapeHtml(i18n.t('module.jitsi_meet.chat.open'))}</a>
      </aside>
...
export async function mount(root, { signal } = {}) {
  ...
  function selectedUsernames() {
    return state.selectedParticipants.map((participant) => participant.username);
  }
  ...
  async function updateChatLink() {
    ...
    if (!(chatHint instanceof HTMLElement) || !(chatLink instanceof HTMLAnchorElement)) {
      return;
    }
  ...
  function renderParticipantTables() {
    const availableBody = root.querySelector('#jitsi-available-participants');
    const selectedBody = root.querySelector('#jitsi-selected-participants');
    if (!(availableBody instanceof HTMLElement) || !(selectedBody instanceof HTMLElement)) {
      return;
    }
  ...
  function addParticipant(entry) {
    if (state.selectedParticipants.some((item) => item.username === entry.username)) {
      return;
    }
    state.selectedParticipants.push(entry);
    state.selectedParticipants.sort((a, b) => a.username.localeCompare(b.username));
  }
  ...
  function applyDrop(username, targetTableId) {
    ...
    if (targetTableId === 'jitsi-available-participants' && fromSelected) {
      ...
      if (!state.availableParticipants.some((entry) => entry.username === normalized)) {
        state.availableParticipants.push(fromSelected);
      }
      state.availableParticipants.sort((a, b) => a.username.localeCompare(b.username));
    }
  ...
  async function loadMeetingState() {
    ...
    const response = await apiFetch('/api/v1/modules/jitsi-meet/meetings/state', {
      method: 'POST',
      headers: {
  ...
  async function joinMeeting() {
    if (!state.meeting?.id) return;
    const joinResponse = await apiFetch('/api/v1/modules/jitsi-meet/meetings/join', {
      method: 'POST',
      headers: {
        'content-type': 'application/json',
      },
    ...
    if (state.meeting.state?.authRequired && !state.meeting.state?.authCompletedAt) {
      updateOverlay({
        message: i18n.t('module.jitsi_meet.overlay.auth_required_description'),
  ...
  async function prepareMeetingStart() {
    ...
    const createResponse = await apiFetch('/api/v1/modules/jitsi-meet/meetings/create', {
      method: 'POST',
      headers: {
        'content-type': 'application/json',
      },
    ...
    const createPayload = await createResponse.json().catch(() => ({ data: null }));
    ...
    const probeResponse = await apiFetch('/api/v1/modules/jitsi-meet/meetings/probe', {
      method: 'POST',
      headers: {
        'content-type': 'application/json',
      },
    ...
    const probePayload = await probeResponse.json().catch(() => ({ data: null }));
  ...
  function bindInteractiveHandlers() {
    ...
    const searchInput = container.querySelector('#jitsi-participant-search-input');
            ...
            .filter(
              (entry) =>
                !state.selectedParticipants.some(
                  (selected) => selected.username === entry.username,
                ),
            )
    ...
    container.addEventListener(
      ...
      (event) => {
        ...
        event.dataTransfer?.setData('text/plain', state.dragUsername ?? '');
      },
      { signal: bindSignal },
    );
    ...
    container.addEventListener(
      ...
      (event) => {
        ...
        const username =
          state.dragUsername ?? event.dataTransfer?.getData('text/plain');
    ...
    if (authButton instanceof HTMLButtonElement) {
      authButton.addEventListener(
        ...
        async () => {
          if (!state.meeting?.id) return;
          await apiFetch('/api/v1/modules/jitsi-meet/meetings/auth-start', {
            method: 'POST',
            headers: {
              'content-type': 'application/json',
            },
          ...
          updateOverlay({
            message: i18n.t('module.jitsi_meet.overlay.auth_in_progress'),
            showAuth: true,
          });
          window.open(state.meeting.meetingUrl, '_blank', 'noopener,noreferrer');
        },
        { signal: bindSignal },
      );
    }
    ...
    if (reclaimButton instanceof HTMLButtonElement) {
      reclaimButton.addEventListener(
        ...
        async () => {
          if (!state.meeting?.id) return;
          await apiFetch('/api/v1/modules/jitsi-meet/meetings/reclaim', {
            method: 'POST',
            headers: {
              'content-type': 'application/json',
            },
          ...
          updateOverlay({
            message: i18n.t('module.jitsi_meet.overlay.reclaim_done'),
            showReclaim: false,
          });
          await openMeetingEmbed();
        },
