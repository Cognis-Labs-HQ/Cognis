...
export function createMountUtilities({ root, state }) {
  ...
  function deferAloneParticipantPrompt(delayMs = ALONE_PROMPT_GRACE_PERIOD_MS) {
  ...
  function updatePreflightIndicator() {
    ...
    indicatorEl.classList.remove('jitsi-spinner', 'jitsi-tick', 'jitsi-cross');
  ...
  function updateOverlay({
    ...
    if (loadingTextEl instanceof HTMLElement && typeof message === 'string') {
      if (loading || probed) {
