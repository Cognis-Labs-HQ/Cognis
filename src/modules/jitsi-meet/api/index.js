...
async function canAccessMeeting({ store, meeting, username }) {
  ...
  const classroomUsernames = await store.listClassroomUsernames(
    meeting.classroomId,
  );
...
export function registerApiRoutes(router, ctx) {
  ...
  const resolveGroupChat = ctx.getCapability('social:messages:resolveGroupChatUrl');
  ...
  if (!store) {
    store = new JitsiMeetStore({
      db: dbExecutor,
      log: ctx.getCapability('logging:log'),
    });
  }
  ...
  router.post('/api/v1/modules/jitsi-meet/config', async (req, res) => {
    await store.ensureSchema();
    ...
    const meetingPrefix = store.normalizeMeetingPrefix(body.meetingPrefix ?? '');
  ...
  router.post('/api/v1/modules/jitsi-meet/meetings/create', async (req, res) => {
    await store.ensureSchema();
    ...
    const payload = await createMeetingPayload({
      ...
      participants,
      requesterUsername,
      chatUrl: chatRoom?.url ?? (meeting.chatRoomId
  ...
  router.post('/api/v1/modules/jitsi-meet/meetings/get', async (req, res) => {
    await store.ensureSchema();
    const claims = requireAuth(req, res, 'user');
    if (!claims) return;
    const body = await readJson(req);
  ...
  router.post('/api/v1/modules/jitsi-meet/meetings/probe', async (req, res) => {
    await store.ensureSchema();
    const claims = requireAuth(req, res, 'user');
    if (!claims) return;
    const body = await readJson(req);
    ...
    const liveness = await checkMeetingLiveness(resolved.meeting.meetingUrl);
    sendJson(res, 200, {
      data: liveness,
    });
  }, { access: { minRole: 'user' } });

  router.post('/api/v1/modules/jitsi-meet/meetings/join', async (req, res) => {
    await store.ensureSchema();
    const claims = requireAuth(req, res, 'user');
    if (!claims) return;
    const body = await readJson(req);
  ...
  router.post('/api/v1/modules/jitsi-meet/meetings/reclaim', async (req, res) => {
    await store.ensureSchema();
    const claims = requireAuth(req, res, 'user');
    if (!claims) return;
    const body = await readJson(req);
  ...
  router.post('/api/v1/modules/jitsi-meet/meetings/presence', async (req, res) => {
    await store.ensureSchema();
    const claims = requireAuth(req, res, 'user');
    if (!claims) return;
    const body = await readJson(req);
  ...
  router.post('/api/v1/modules/jitsi-meet/meetings/auth-required', async (req, res) => {
    await store.ensureSchema();
    const claims = requireAuth(req, res, 'user');
    if (!claims) return;
    const body = await readJson(req);
  ...
  router.post('/api/v1/modules/jitsi-meet/meetings/auth-start', async (req, res) => {
    await store.ensureSchema();
    const claims = requireAuth(req, res, 'user');
    if (!claims) return;
    const body = await readJson(req);
  ...
  router.post('/api/v1/modules/jitsi-meet/meetings/auth-complete', async (req, res) => {
    await store.ensureSchema();
    const claims = requireAuth(req, res, 'user');
    if (!claims) return;
    const body = await readJson(req);
  ...
  router.post('/api/v1/modules/jitsi-meet/meetings/state', async (req, res) => {
    await store.ensureSchema();
    const claims = requireAuth(req, res, 'user');
    if (!claims) return;
    const body = await readJson(req);
  ...
  router.get('/api/v1/modules/jitsi-meet/admin/meetings', async (req, res) => {
    await store.ensureSchema();
    const claims = requireAuth(req, res, 'admin');
    if (!claims) return;
