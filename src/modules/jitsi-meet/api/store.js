...
function normalizeMeetingPrefix(rawPrefix) {
  const value = String(rawPrefix ?? '').trim().toLowerCase();
  if (!value) return '';
  return value
    .replace(/[^a-z0-9-]/g, '-')
    .replace(/-+/g, '-')
...
export class JitsiMeetStore {
  ...
  async ensureSchema() {
    ...
    await this.db.ensureTable({
      ...
      columns: [
        { name: 'id', type: 'text', primaryKey: true },
        { name: 'participant_key', type: 'text', unique: true, notNull: true },
        { name: 'meeting_url', type: 'text', unique: true, notNull: true },
        { name: 'meeting_password', type: 'text', notNull: true },
        { name: 'meeting_name', type: 'text', notNull: true, default: 'Cognis Classroom' },
    ...
    await this.db.ensureTable({
      ...
      columns: [
        ...
        { name: 'auth_required', type: 'integer', notNull: true, default: 0 },
  ...
  async getConfig() {
    ...
    return {
      ...
      meetingPrefix: row?.meeting_prefix ? String(row.meeting_prefix) : '',
  ...
  async createMeeting({
    ...
    if (!normalizedInstanceUrl) {
      throw new Error('A valid Jitsi instance URL is required before creating meetings.');
    }
  ...
  async listClassroomUsernames(classroomId) {
    const normalizedClassroomId = String(classroomId ?? '').trim();
    if (!normalizedClassroomId) return [];

    const studentRows = await this.db.executeCommand({
      option: 'SELECT',
      table: 'class_memberships',
      alias: 'cm',
      columns: [{ col: 'ap.handle', as: 'handle' }],
      joins: [
        {
          type: 'INNER',
          ...
          alias: 'ap',
          on: {
            leftColumn: 'ap.account_id',
            rightColumn: 'cm.student_account_id',
          },
        },
      ],
      where: [
        { column: 'cm.class_id', value: normalizedClassroomId },
        { column: 'cm.status', value: 'member' },
      ],
    });

    const teacherRows = await this.db.executeCommand({
      option: 'SELECT',
      table: 'study_classes',
      alias: 'sc',
      columns: [{ col: 'ap.handle', as: 'handle' }],
      joins: [
        {
          ...
          on: {
            leftColumn: 'ap.account_id',
            rightColumn: 'sc.teacher_account_id',
          },
        },
      ],
      where: [{ column: 'sc.id', value: normalizedClassroomId }],
      limit: 1,
    });

    return normalizeUsernames([
      ...(studentRows.rows ?? []).map((row) => String(row.handle ?? '')),
      ...(teacherRows.rows ?? []).map((row) => String(row.handle ?? '')),
    ]);
  }
  ...
  async getMeetingState(meetingId) {
    ...
    return {
      meetingId,
      firstJoinedBy: row.first_joined_by ? String(row.first_joined_by) : null,
      firstJoinedAt: row.first_joined_at ? String(row.first_joined_at) : null,
      authRequired: Number(row.auth_required ?? 0) === 1,
      authStartedBy: row.auth_started_by ? String(row.auth_started_by) : null,
      authStartedAt: row.auth_started_at ? String(row.auth_started_at) : null,
      authCompletedAt: row.auth_completed_at ? String(row.auth_completed_at) : null,
      updatedAt: row.updated_at ? String(row.updated_at) : null,
    };
  }
  ...
  normalizeMeetingCreationInput({ participants, classroomId, creatorUsername }) {
