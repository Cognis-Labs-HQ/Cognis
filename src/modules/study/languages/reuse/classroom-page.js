...
export async function mountStudyClassroomPage(root, { signal, languageCode }) {
    ...
    const viewerAccountId = String(localStorage.getItem('cognis_account') ?? '').trim();
    ...
    async function loadClassrooms() {
        const languageQuery = languageCode
            ...
            : '';
        const response = await apiFetch(`/api/v1/study/classrooms${languageQuery}`);
        if (!response.ok) {
        ...
        if (!selectedClassId || !classroomSnapshots.some((snapshot) => snapshot.id === selectedClassId)) {
    ...
    async function persistClassroomState(partialState) {
        ...
        const payload = await response.json();
        selectedSnapshot.classroom = payload?.data ?? selectedSnapshot.classroom;
    }
    ...
    async function moveStudent(studentAccountId, newSeatNumber) {
        ...
        await persistClassroomState({
            seatAssignments,
            studentLimit: Number(selectedSnapshot?.classroom?.studentLimit ?? 30),
        });
    }
    ...
    function renderClassOptions() {
        ...
        return classroomSnapshots
            .map((snapshot) => {
                const selectedAttribute = snapshot.id === selectedClassId ? ' selected' : '';
    ...
    function renderClassroom() {
        ...
        const studentLimit = Number(selectedSnapshot?.classroom?.studentLimit ?? 30);
        const normalizedStudentLimit = Number.isInteger(studentLimit) && studentLimit > 0
            ? studentLimit
            : 30;
        const seatAssignments = normalizeSeatAssignments(
            selectedSnapshot?.classroom?.seatAssignments,
        ...
        const seatTiles = Array.from({ length: normalizedStudentLimit })
            .map((_, seatNumber) => {
                ...
                const draggableAttribute =
                    roleFlags.isTeacher && studentAccountId ? ' draggable="true"' : '';
                return `
                    <button
                        type="button"
        ...
        const selectedSeatStudentAccountId =
            selectedSeatNumber != null ? studentBySeatNumber.get(selectedSeatNumber) : '';
        ...
        elements: [
            {
                ...
                onRender: () => {
                    ...
                    if (!(classroomContentElement instanceof HTMLElement)) return;
                    ...
                    classSelectElement.addEventListener(
                        'change',
                        () => {
                            selectedClassId = String(classSelectElement.value ?? '');
                    ...
                    classroomContentElement.addEventListener(
                        ...
                        async (event) => {
                            ...
                            if (removeStudentButton instanceof HTMLElement) {
                                ...
                                try {
                                    await removeStudent(selectedStudentAccountId);
                                    showToast(
                                        i18n.t(
                            ...
                            if (leaveButton instanceof HTMLElement) {
                                try {
                                    ...
                                    showToast(
                                        i18n.t('module.study.classes.leave_success'),
                                        { variant: 'success' },
                                    );
                                    refreshClassroomContent();
                                } catch (error) {
                    ...
                    classroomContentElement.addEventListener(
                        ...
                        (event) => {
                            ...
                            if (!(seatButtonElement instanceof HTMLElement)) return;
                            ...
                            event.dataTransfer?.setData('text/plain', studentAccountId);
                        },
                        { signal },
                    );
