...
| `external` | Uploaded `.zip` or `.tar.gz` archive           | Via admin API or `component:import-github` CLI | Displayed before enabling |
...
1. Admin submits `repositoryUrl` and `versionTag` via Administration UI or `cognisctl component:import-github`.
