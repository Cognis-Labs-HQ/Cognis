...
test('module lifecycle writes nginx-style pointer for internal modules', async () => {
  ...
  await mkdir(path.join(internalModulesPath, 'sample-analytics'), { recursive: true });

  const service = new ModuleService(runtime([
    { id: 'sample-analytics', name: 'Analytics', version: '1.0.0', class: 'extension', coreApiVersion: 'v1', capabilities: [], entrypoints: {} }
  ]), { internalModulesPath, externalModulesPath, enabledPointersPath });

  await service.enable('sample-analytics');
  const target = await readlink(path.join(enabledPointersPath, 'sample-analytics.load'));
...
test('external modules require disclaimer acknowledgement', async () => {
  ...
  await writeFile(path.join(externalModulesPath, 'weather-pack.zip'), 'dummy');

  const service = new ModuleService(runtime([
    { id: 'weather-pack', name: 'Weather', version: '1.0.0', class: 'extension', coreApiVersion: 'v1', capabilities: [], entrypoints: {} }
  ]), { internalModulesPath, externalModulesPath, enabledPointersPath });

  await assert.rejects(() => service.enable('weather-pack'));
  const enabled = await service.enable('weather-pack', { acknowledgeExternalDisclaimer: true });
