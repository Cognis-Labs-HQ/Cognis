...
プロバイダーはサーフェスメタデータに `minSize: { width, height }` を含め、PiP の最小寸法をピクセル単位で宣言できます。両方の寸法は正の有限数でなければなりません。ホストはサイズ変更時に適用するため、それらをフローティングウィンドウコントローラーへ渡します。`minSize` を省略した場合はホストの既定値が使用されます。
