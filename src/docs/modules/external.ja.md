...
既定では、ブートストラップの失敗は有効化を中止し、モジュールを無効な状態に戻します。任意の起動処理が安全に失敗できるモジュールは、`manifest.json` に `allowBootstrapFailure: true` を明示できます。その場合、Cognis はモジュールを有効なままにし、不完全な実行時提供を削除して、失敗を警告として記録します。この例外指定は、失敗したブートストラップによって提供されない機能とルートがすべて本当に任意である場合にのみ使用してください。
