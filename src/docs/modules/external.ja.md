...
モジュールは、`ui.preferences` を使用して管理者が編集可能な設定を公開できます。各定義には、安定した `key`、ローカライズされた `label`、オプションの `description`、`boolean`、`string`、または `number` の `type`、およびオプションの一致する `default` が含まれます。 Cognis は、定義が存在する場合にのみインストール済みモジュールの詳細ビューに設定を表示し、モジュールランタイムで共有する一つの運用設定を読み込み、宣言されたキーへの書き込みを絞り込み、設定ストア機能を通じて永続化します。モジュールのブートストラップとリクエストハンドラーは、具体的なストアをインポートしたり並行する古い設定を参照したりせず、`ctx.preferences.get()` で同じ最新値を読み取ります。
