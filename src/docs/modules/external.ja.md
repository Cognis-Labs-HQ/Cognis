...
モジュールは `ui.preferences` で管理者が編集できる設定を公開できます。各フィールドは、安定した `key`、ローカライズ用の `labelKey`、任意の `descriptionKey`、`boolean`、`string`、`number` のいずれかの `type`、任意の対応する `default`、機密文字列を隠す `secret: true`、およびモジュール所有の設定エンドポイントが値を返すまで有効化をブロックする場合の `required: true` を宣言し、`ui.stringsBaseUrl` がモジュール所有の翻訳を示します。Cognis はこのマニフェスト契約をインストール済みモジュールの詳細画面に描画し、`GET /api/v1/modules/<id>/config` をポーリングして、変更を同じモジュール所有エンドポイントへ `PUT` します。モジュールは運用設定を検証、適用、永続化します。別の設定 UI を提供したり、Cognis のユーザー設定を構成保存先として使用したりしてはいけません。 モジュールの無効化や再起動では、この永続化された設定を保持しなければならず、削除できるのはアンインストールフローだけです。
