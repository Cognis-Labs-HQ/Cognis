...
Cognis はモジュール所有のテスト実行または外部モジュールの有効化より前に、チェックアウト内のすべての JavaScript、TypeScript、CSS ファイルを検査します。Cognis の内部実装をインポートするコード、内部の静的 URL や API URL を埋め込むコード、コアシェルまたは再利用 UI カタログ所有のクラスを装飾する CSS がある場合、有効化は拒否されます。ホストとの連携は、明示的に公開された `ctx` または `uiCtx` の機能だけを通じて行う必要があります。
