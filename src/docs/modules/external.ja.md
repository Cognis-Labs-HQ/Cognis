...
認証前にも必要な法的文書、サポート文書、その他の文書を公開するモジュールは、`ctx.registerAuthFooterPlugin({ scriptUrl })` を呼び出せます。Cognis はログインページと登録ページの両方で有効なコントリビューターを読み込みます。ブラウザーモジュールは `ui:footerLinks` を通じてリンクを提供し、Cognis は認証パネル下部の共通ストリップに表示します。
