...
モジュールページのコードが所有できるのは、`mount()` 関数に渡されたルートだけです。`document.body`、`document.head`、ダッシュボードシェル、ナビゲーション、アカウント操作を変更してはいけません。グローバルな動作は、取り外し可能なフックを備えた宣言済みの `uiCtx` 機能またはフローとして提供します。宣言された `/static/modules/` のスタイルシートは低いカスケードレイヤーに配置されるため、ホストシェルとプロフィールのスタイルが優先され、モジュールスタイルはページ表示領域内の制御を維持します。
