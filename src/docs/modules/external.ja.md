...
モジュールは、`ui.preferences` を使用して管理者が編集可能な設定を公開できます。各定義には、安定した `key`、ローカライズされた `label`、オプションの `description`、`boolean`、`string`、または `number` の `type`、およびオプションの一致する `default` が含まれます。 Cognis は、定義が存在する場合にのみインストール済みモジュールの詳細ビューに設定を表示し、サインインしている管理者用に値をロードし、宣言されたキーへの書き込みをフィルタリングし、設定ストア機能を通じてそれらを永続化します。モジュールは、具体的なストアをインポートするのではなく、ctx ベースのモジュール設定機能を通じて値を読み取る必要があります。
