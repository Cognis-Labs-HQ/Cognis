...
Module, die rechtliche, unterstützende oder andere vor der Anmeldung relevante Dokumente veröffentlichen, können `ctx.registerAuthFooterPlugin({ scriptUrl })` aufrufen. Cognis lädt aktivierte Beiträge auf der Anmelde- und Registrierungsseite. Das Browsermodul stellt Links über `ui:footerLinks` bereit, und Cognis zeigt sie im gemeinsamen Streifen unterhalb der Authentifizierungsbereiche an.
