...
Sebuah modul dapat menampilkan pengaturan yang dapat diedit administrator dengan `ui.preferences`. Setiap definisi berisi `key` yang stabil, `label` yang dilokalkan, `description` opsional, `type` dari `boolean`, `string`, atau `number`, dan `default` opsional yang cocok. Cognis memperlihatkan Pengaturan dalam tampilan detail modul yang diinstal hanya ketika definisi ada, memuat nilai untuk administrator yang masuk, memfilter penulisan ke kunci yang dideklarasikan, dan menyimpannya melalui kemampuan penyimpanan preferensi. Modul harus membaca nilainya melalui kemampuan preferensi modul yang didukung ctx daripada mengimpor penyimpanan nyata.
