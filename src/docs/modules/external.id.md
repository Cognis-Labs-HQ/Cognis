...
Modul yang menerbitkan dokumen hukum, dukungan, atau dokumen lain yang relevan sebelum autentikasi dapat memanggil `ctx.registerAuthFooterPlugin({ scriptUrl })`. Cognis memuat kontributor aktif pada halaman masuk dan pendaftaran; modul peramban menyumbangkan tautan melalui `ui:footerLinks`, lalu Cognis menampilkannya pada bilah bersama di bawah panel autentikasi.
