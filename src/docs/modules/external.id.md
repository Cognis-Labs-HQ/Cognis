...
Sebuah modul dapat menampilkan pengaturan yang dapat diedit administrator dengan `ui.preferences`. Setiap definisi berisi `key` yang stabil, `label` yang dilokalkan, `description` opsional, `type` dari `boolean`, `string`, atau `number`, dan `default` opsional yang cocok. Cognis memperlihatkan Pengaturan dalam tampilan detail modul yang diinstal hanya ketika definisi ada, memuat satu konfigurasi operasional yang dibagikan oleh runtime modul, memfilter penulisan ke kunci yang dideklarasikan, dan menyimpannya melalui kapabilitas penyimpanan preferensi. Bootstrap modul dan penangan permintaan membaca nilai terkini yang sama dengan `ctx.preferences.get()`, bukan mengimpor penyimpanan konkret atau membaca konfigurasi lama yang terpisah.
