...
A module can expose administrator-editable settings with `ui.preferences`. Each definition contains a stable `key`, localized `label`, optional `description`, a `type` of `boolean`, `string`, or `number`, and an optional matching `default`. Cognis shows Settings in the installed-module detail view only when definitions exist, loads one operational configuration shared by the module runtime, filters writes to declared keys, and persists it through the preference-store capability. Module bootstrap and request handlers read the same current values with `ctx.preferences.get()` rather than importing a concrete store or consulting a parallel legacy configuration.
