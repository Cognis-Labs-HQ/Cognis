...
Capability spawn mengembalikan handle dengan `discard()`. Pemanggil wajib membuangnya saat aksi tutup atau kembali digunakan; abort signal juga akan membuangnya. Sebagai alternatif, `component-pages:discard` membuang jendela berdasarkan ID panggung. Pemeriksaan ketersediaan saat halaman rapat dimuat hanya boleh memakai `component-pages:request`. Penyedia wajib merender hanya di root yang diberikan, menghormati signal, melepaskan sumber dayanya ketika dibuang, dan tidak menjalankan navigasi langsung saat disematkan.
