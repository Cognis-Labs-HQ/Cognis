...
ブラウザーは、グループ化されたページの検出、キャッシュ無効化、パス解決を行う汎用 `ui:subPages` 機能を提供します。Study ゲートウェイは `study:subPages` を通じて言語をグループ、各言語の子コンポーネントをページとして登録します。ハブ、選択言語のサブメニュー、子ルートローダーは同じモデルを利用するため、ページ一覧とナビゲーションがずれません。
