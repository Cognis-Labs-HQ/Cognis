...
A surface that declares `pip` is presented through Cognis' reusable floating-window behavior. Each floating window includes a thin host-owned toolbar that can be dragged from anywhere along its top edge and visible SVG resize handles in its upper-left and lower-right corners; Cognis keeps the window within the visible viewport and releases all listeners when the focus session ends. Provider modules normally only declare `pip` and mount into the supplied root. A module that owns a separate PiP element, such as a meeting frame, can obtain `ui:makeFloatingWindow` from `uiCtx.capabilities`, pass its element, drag handle, and page signal, and retain the returned cleanup function. Modules must not import the utility directly or install competing document-level drag or resize handlers.

A provider can include `minSize: { width, height }` in its surface metadata to declare the PiP's minimum dimensions in pixels. Both dimensions must be positive finite numbers. The host passes them to the floating-window controller for enforcement during resizing; when `minSize` is omitted, the host defaults apply. A provider that directly creates a floating window can call `release.updateMinimumSize({ width, height })` on the returned cleanup function. Valid updates immediately enlarge an undersized window and keep it within the available boundary; the method returns `false` for invalid updates or after cleanup. During a resize, dragging the window below its minimum width while extending it beyond its minimum height switches those minimum dimensions to a vertical orientation. Dragging a vertically oriented window beyond its minimum width while reducing it below its minimum height switches back to the default horizontal orientation.
