...
A provider can include `minSize: { width, height }` in its surface metadata to declare the PiP's minimum dimensions in pixels. Both dimensions must be positive finite numbers. The host passes them to the floating-window controller for enforcement during resizing; when `minSize` is omitted, the host defaults apply.
