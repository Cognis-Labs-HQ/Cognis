...
Classroom ページは、プロフィールアバターヘルパーをハードなゲートウェイ
import ではなく動的に読み込むようになりました。Social ゲートウェイの UI
アセットがなくても `/classroom` は開き続け、アバターのハイドレーション
だけが静かにフォールバックします。

## レビューフィードバックに基づく改善

このセクションでは、このプルリクエストのレビューフィードバックに対応して
行われた変更を記録します。

## ホワイトボードモジュールがクラスルーム埋め込みアクセスを管理

クラスルームアダプターはJWTの生成やアクティブボードの可視性チェックを
内部で処理しなくなりました。Nextcloud Whiteboardモジュールの新しい
`whiteboard:getClassroomBoardEmbed`ケイパビリティが、完全なアクセス制御フロー
（`study:classes:resources`ケイパビリティを通じた生徒向けの教師アクティブ
ボードチェック）を処理し、埋め込みURLまたはエラーコードを返します。
クラスルームルートはこのケイパビリティを呼び出すだけで済みます。
`getActiveWhiteboardId`ヘルパーが`study:classes:resources`ケイパビリティ
コントラクトに追加されました。

## ホワイトボードモジュールが環境変数を使用しなくなった

Nextcloud Whiteboardモジュールは、データベースベースの設定UIからのみ
設定を読み取るようになりました。`NEXTCLOUD_WHITEBOARD_URL`、
`NEXTCLOUD_WHITEBOARD_SECRET`、`NEXTCLOUD_WHITEBOARD_TOKEN_EXPIRY_SECONDS`の
環境変数が`docker/Dockerfile`から削除されました。管理者は管理画面→
モジュールパネルからモジュールを設定します。

## 汎用ドラッグカーソルクラスをreuse CSSに移動

`.can-drag`と`.can-drag.is-dragging`ユーティリティクラスが
`src/ui/styles/reuse/layout.css`に移動されました。
画像ビューアーはコンポーネント固有のドラッグカーソルルールを定義しなく
なりました。

## 画像ビューアーのズーム感度と範囲

ピンチ/スクロールのズーム係数が1.12から1.07に引き下げられました。
ズーム範囲は0.25×〜5×に制限されました（以前は0.1×〜10×）。

## クラスルームノートパッドのURLネームスペースを修正

クラスルームのノートパッドとアジェンダのAPIルートがすべて
`/api/v1/study/classes/:id/`から
`/api/v1/file-reader/text/classroom-notes/:id/`に移動しました。
これにより、コンポーネント間のURL知識の違反が解消されました。

## FileGatewayLikeを削除

classroom-files-route.tsとtext/routes/index.tsのアドホック
`FileGatewayLike`インターフェースが、`src/core/contracts/files-gateway.ts`の
正規`FileStorageGateway`コントラクトに置き換えられました。

## グループチャットの管理者ロール

グループチャットルームがメンバーとオーナーの中間となる**管理者**ロールを
サポートするようになりました。ルームオーナーは新しい
`PATCH /api/v1/social/messages/rooms/:id/members/:selector/role`エンドポイントで
メンバーを管理者に昇格または降格できます。管理者は通常のメンバーを削除・
ミュートできますが、他の管理者やオーナーには作用できません。
ダイレクトメッセージは変更なしです。

## グループチャットのアバターグリッドレイアウトを修正

3メンバーのアバターグリッドは、最初のメンバーを上段全体に表示し、
残り2人を下段に並べるようになりました。4メンバーのレイアウトは
標準の2×2グリッドを使用します。

## テキストアダプターの最大ファイルサイズを設定可能に

Text File Readerアダプターが受け入れる最大ファイルサイズをデータベースに
保存するようになりました。管理者は管理画面→アダプターパネルの
File Readerゲートウェイから値を確認・変更できます。設定は16KBから4MB
（デフォルト256KB）の範囲に制限されます。

## File Readerゲートウェイのアダプター一覧
