...
The published recommendation UUIDs remain in the default catalog. Marketplace cards, details, categories, and tags now resolve localization keys from each manifest through its declared module string bundle. Modules that omit `ui.stringsBaseUrl` now use the standard `ui/languages` bundle automatically, and unresolved literals remain visible instead of becoming empty text. Module artwork object URLs now remain cached across SPA transitions to prevent fallback-image flashes, and installed README variants follow the active UI language with English fallback. The installer no longer treats the optional root `README.md` compatibility alias as a required integrity file, allowing it to be a symlink to a localized English README. Scoped share guests now load active host UI capability providers after authentication and before a shared module mounts, restoring profile-avatar rendering without making provider requests before a guest token exists.
