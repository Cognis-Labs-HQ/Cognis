...
カード更新で、順序のない参照を無効な位置情報なしに保持するようになりました。関係タブは表示専用になり、複合文字では自由な入力欄と発音カルーセルを使います。検索操作は入力後に同じ行へ現れ、ストロークパターンはプロバイダー所有の非表示データとして維持されます。定義追加操作は大きなニュートラル表示になり、プレビュー背景が内容全体を包みます。
...
認証済みページでブラウザ生成のメディア URL を許可し、新しくアップロードしたカード音声をコンテンツセキュリティポリシー違反なしで再生できるようにしました。既存の複合文字カードでは、コンテンツタブにプロバイダー表記の発音候補を表示します。編集時は作成時の参照位置で並べてから番号を付け、同じ対象レイヤーの重複カルーセルを除き、順序付き文字選択から発音を直接導出します。無関係なレイヤーと入れ子作成操作は表示しません。
