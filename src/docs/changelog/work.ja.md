...
リーダーボードの状態を永続化し、証拠イベントを不変として時間枠に照合し、完全な同順位と設定済みの同点決着規則を順位に反映するようにしました。定義や順位を返す前にクラス、コホート、イベントへのアクセスを認可し、送信、順位付け、シーズン更新にはフローフックが参加します。

## 正確でローカライズされた Study UI

リーダーボードページで要求エラーを通知し、アクセス可能なすべてのリーダーボードを選択でき、各列に対応する値を表示し、表のアクセシビリティ情報をローカライズするようにしました。Study ナビゲーションは登録済みルートから利用可否を判断し、別言語のラベルを代用しません。
