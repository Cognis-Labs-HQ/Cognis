...
Invitation expiry time now appears in a compact pill. The introduction and form cards also size themselves independently, so a long creation form no longer leaves an unnecessarily tall card on the left.

## Keep public pages free of account requests

Availability and presence reporting now waits for an authenticated account token, preventing public authentication pages from making account-only Social API requests.
