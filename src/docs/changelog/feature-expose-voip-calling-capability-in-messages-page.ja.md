...
## 接続を維持する PiP と通話画面の自動クリーンアップ

通話コンポーネントはピクチャーインピクチャーへの移行時と復帰時に状態保持 DOM 移動を使用し、埋め込み Jitsi 会議が移行中に再接続されるのを防ぎます。コンポーネントの削除はマウント直後から監視されるため、プロバイダー側で会議が終了すると空の通話枠が取り除かれ、チャットが復元されます。
