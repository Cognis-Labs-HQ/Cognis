...
CLI ブートストラップトークンは通常ユーザー ID ではなくシステム主体を使うようになりました。ユーザー所有データを参照または作成する TFA とカレンダーのコマンドは明示的なユーザー名を必須にし、`cognisctl` が自分自身の既定カレンダー、TFA レコード、その他のユーザー単位状態を作成しないようにしました。
