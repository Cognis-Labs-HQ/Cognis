...
async function showPreview(link) {
    ...
    preview.innerHTML = `<div class="profile-mini-preview__loading">${escapeHtml(previewI18n?.t("ui.reuse.profile_preview_loading") ?? "…")}</div>`;
    ...
    const stats = [
        profile.followerCount != null
            ? `${profile.followerCount} ${previewI18n?.t("ui.reuse.profile_preview_followers") ?? ""}`
            : null,
        profile.followingCount != null
            ? `${profile.followingCount} ${previewI18n?.t("ui.reuse.profile_preview_following") ?? ""}`
            : null,
        profile.postCount != null
            ? `${profile.postCount} ${previewI18n?.t("ui.reuse.profile_preview_posts") ?? ""}`
            : null,
    ]
        .filter(Boolean)
        .join(" · ");
