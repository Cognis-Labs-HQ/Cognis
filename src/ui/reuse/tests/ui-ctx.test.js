...
function loadUiCtxForTests() {
  const source = readFileSync(resolve(ROOT, 'src/ui/reuse/ui-ctx.js'), 'utf8');
...
test('createFlowEngine — runFlow executes hooks in stage order', async () => {
  ...
  engine.extendFlow('ordered-flow', 'alpha', { id: 'hook-alpha' }, async () => {
    calls.push('alpha');
    return { ran: 'alpha' };
  });
...
test('createFlowEngine — stageCtx.data is shared across hooks within the same run', async () => {
  ...
  engine.extendFlow('data-flow', 'read', { id: 'reader' }, async (stageCtx) => {
