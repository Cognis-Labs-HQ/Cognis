 ...
 *     groups: [{ id: 'legal', label: 'Legal', items: [{ id: 'terms', label: 'Terms' }] }],
 *     storageKeyPrefix: 'legal-menu',
 *     onSelect: (id) => showDocument(id),
 *   });
 *   container.innerHTML = menu.render();
 *   menu.mount(container);
 *
 * @param {{ groups: Array<{id: string, label: string, items: Array<{id: string, label: string}>}>, storageKeyPrefix: string, onSelect?: (id: string) => void, activeId?: string }} options
            ...
            .map((group) => {
                ...
                return `<details class="side-menu-group"${isOpen ? " open" : ""} data-side-menu-group="${escapeHtml(groupId)}"><summary>${escapeHtml(String(group?.label ?? ""))}</summary><ul>${items
                    .map((item) => {
                        ...
                        return `<li><button class="side-menu-link${isActive ? " active" : ""}" data-side-menu-item="${escapeHtml(itemId)}"${isActive ? ' aria-current="page"' : ""}>${escapeHtml(String(item?.label ?? ""))}</button></li>`;
                    })
                    .join("")}</ul></details>`;
            })
            .join("")}</nav>`;
