...
export interface ProfileCreateStore {
  createProfile(accountId: string, handle: string, role?: AccountRole): Promise<AccountProfile | null>;
...
export class DbProfileStore implements ProfileCreateStore {
  ...
  async createPost(
    ...
    await this.db.execute(
      ...
      [id, accountId, input.title ?? null, input.content, input.visibility]
    );
    const result = await this.db.execute(
  ...
  async getAllFileSizeLimits(): Promise<FileSizeLimit[]> {
    const result = await this.db.execute('SELECT category, max_bytes FROM file_size_limits ORDER BY category');
    return (result.rows ?? []).map((row) => ({ category: row.category, maxBytes: Number(row.max_bytes) }));
  }
}
