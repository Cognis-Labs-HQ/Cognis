...
export class DbUserPreferenceStore {
  ...
  private placeholder(index: number) { return this.dbType === 'postgresql' ? `$${index}` : '?'; }
  ...
  async clearUser(accountId: string) {
    await this.db.execute(`DELETE FROM user_preferences WHERE account_id = ${this.placeholder(1)}`, [accountId]);
  }
}
