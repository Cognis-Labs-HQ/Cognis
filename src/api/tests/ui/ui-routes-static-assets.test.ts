...
test('versioned text assets negotiate Brotli with MIME and immutable caching', async () => {
  ...
  try {
    ...
    await route(
      ...
      recorder.res as any,
      new URL('http://localhost/static/reuse/api-client.js?v=development'),
    );
...
test('GET /static/gateways/:id/:file serves file from registered static dir', async () => {
  ...
  const handled = await route(
    { headers: {} } as any,
    recorder.res as any,
    new URL('http://localhost/static/gateways/auth/security-prefs/index.js'),
  );
...
test('GET /static/adapters/share/:method/page.js serves Share adapter pages', async () => {
  ...
  for (const adapterId of ['link', 'user']) {
    ...
    const handled = await route(
      { headers: {} } as any,
      recorder.res as any,
      new URL(`http://localhost/static/adapters/share/${adapterId}/page.js`),
    );
