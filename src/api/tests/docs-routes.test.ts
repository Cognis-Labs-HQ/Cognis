...
test('docs route handles docs index with markdown slugs', async () => {
  ...
  assert.match(body, /"slug":"docs\/components\/ui"/);
});
...
test('docs route supports slug lookup', async () => {
  ...
  let status = 0;
  let body = '';
  const handled = await route({ method: 'GET' } as any, { writeHead(code: number) { status = code; }, end(payload: string) { body = payload; } } as any, new URL('http://localhost/api/v1/docs/docs/components/ui'));
  assert.equal(handled, true);
  assert.equal(status, 200);
