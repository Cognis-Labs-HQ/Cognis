...
test('post routes - unauthenticated POST /api/v1/posts returns 401', async () => {
  ...
  try {
    ...
    await route(
      makeReq('POST', null, JSON.stringify({ content: 'hello', visibility: 'community' })),
      { writeHead(c: number) { status = c; }, end() {} } as any,
      new URL('http://localhost/api/v1/posts')
    );
...
test('post routes - invalid visibility returns 400', async () => {
  ...
  try {
    ...
    await route(
      makeReq('POST', token, JSON.stringify({ content: 'hello', visibility: 'public' })),
      { writeHead(c: number) { status = c; }, end() {} } as any,
      new URL('http://localhost/api/v1/posts')
    );
    assert.equal(status, 400);
...
test('post routes - moderator can delete another user post', async () => {
  ...
  try {
    ...
    await route(
      makeReq('POST', aliceToken, JSON.stringify({ content: 'removable post', visibility: 'community' })),
      { writeHead(c: number) { status = c; }, end(p: string) { body = p; } } as any,
      new URL('http://localhost/api/v1/posts')
    );
    const postId = JSON.parse(body).data.id;
...
test('post routes - admin can delete another user post', async () => {
  ...
  try {
    ...
    await route(
      makeReq('POST', aliceToken, JSON.stringify({ content: 'post to remove', visibility: 'community' })),
      { writeHead(c: number) { status = c; }, end(p: string) { body = p; } } as any,
      new URL('http://localhost/api/v1/posts')
    );
    const postId = JSON.parse(body).data.id;
...
test('post routes - private account posts only visible to followers', async () => {
  ...
  try {
    ...
    await route(
      makeReq('POST', aliceToken, JSON.stringify({ content: 'private post', visibility: 'private' })),
      { writeHead(c: number) { status = c; }, end(p: string) { body = p; } } as any,
      new URL('http://localhost/api/v1/posts')
    );
