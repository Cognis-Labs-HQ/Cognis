...
test("buildServer keeps public requests responsive while runtime restoration is pending", async () => {
    const runtimeStatePending = new Promise<never>(() => {});
    const server = buildServer({
        moduleRuntimeGateway: {
            listManifests: () => runtimeStatePending,
        } as unknown as ModuleRuntimeGateway,
        routeContext: createDefaultRouteContext(),
    });
    ...
    try {
        ...
        assert.equal(rootResponse.headers.get("location"), "/dashboard");

        for (const pathname of [
            "/api/v1/system/health",
            "/api/v1/system/healthcheck",
            "/api/v1/ui/auth-typing-messages",
            "/sw.js",
