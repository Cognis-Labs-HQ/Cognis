...
async function resolveLoginRedirectLocation(
  ...
  if (accountId && accountStore && typeof accountStore.getInfo === 'function') {
...
export function createUiRoutes(
  ...
  return async (
    ...
    if (url.pathname === '/api/v1/ui/asset-manifest' && req.method === 'GET') {
      res.writeHead(200, {
    ...
    if (url.pathname === '/sw.js' && req.method === 'GET') {
      try {
        const file = await readFile(path.join(SERVED_PUBLIC_ROOT, 'sw.js'));
        res.writeHead(200, {
    ...
    if (url.pathname === '/invite') {
      ...
      if (!registrationGateway || registrationGateway.status === 'disabled') {
        res.writeHead(302, { location: '/dashboard' });
        res.end();
        return true;
      }
    ...
    if (url.pathname.startsWith('/changelogs')) {
      ...
      await serveHtmlPageWithReplacements(
        ...
        [
          ...
          {
            from:
              uiRegistry?.resolveAssetUrl('/static/app/docs/index.js') ??
              '/static/app/docs/index.js',
            to:
              uiRegistry?.resolveAssetUrl('/static/app/changelogs/index.js') ??
    ...
    if (runtime && !url.pathname.startsWith('/api/')) {
      ...
      for (const manifest of manifests) {
        ...
        try {
          ...
          const routes = parseModuleUiRoutes(await readFile(routeFile, 'utf8'));
          ...
          log?.('error', 'Failed to resolve module UI route declarations.', {
            component: 'api-ui',
            moduleId: manifest.id,
            path: url.pathname,
            error: error instanceof Error ? error.message : String(error),
    ...
    if (pageExtMatch && req.method === 'GET') {
      ...
      const extensions = (uiRegistry?.listPageExtensions(pageId) ?? []).filter(
        (extension) =>
    ...
    if (
      ...
      const gatewayMessages = (uiRegistry?.listAuthTypingMessages() ?? [])
        .filter((message) => {
          ...
          if (
            ...
            message.ownerId &&
            gatewayRegistry?.get(message.ownerId)?.status === 'disabled'
          ) {
            return false;
          }
          if (
      ...
      const moduleMessages = runtime
        ? (await runtime.listManifests())
            ...
            .flatMap((manifest) =>
              (manifest.ui?.authTypingMessages ?? []).map((textKey, index) => ({
    ...
    if (
      ...
      if (ctx.flow.exists('construct-settings-ui')) {
        ...
        const flowSections = result.data['sections'] as unknown[] | undefined;
    ...
    if (url.pathname === '/api/v1/ui/navbar-plugins' && req.method === 'GET') {
      const claims = ctx.requireAuth(req, res, 'user');
      if (!claims) return true;
    ...
    if (url.pathname.startsWith('/static/adapters/')) {
      ...
      if (parts.length >= 3) {
        ...
        const dir = uiRegistry?.getAdapterStaticDir(gatewayId, adapterId);
        if (
          dir &&
    ...
    if (relative === null) {
      if (!url.pathname.startsWith('/api/') && req.method === 'GET') {
        const requestedCode = Number(url.searchParams.get('code') ?? '');
        const errorCode =
