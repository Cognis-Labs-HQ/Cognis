...
function normalizeErrorPageCode(value: unknown): string | null {
    const rawValue = String(value ?? "").trim();
    if (!/^\d{3}$/.test(rawValue)) return null;
    const numericCode = Number(rawValue);
    if (numericCode < 400 || numericCode > 599) return null;
    return rawValue;
}

function resolveUiRouteErrorCode(error: unknown): string {
    const statusCandidate =
        typeof error === "object" && error !== null
            ...
            : null;
    const normalizedStatusCode = normalizeErrorPageCode(statusCandidate);
    ...
    const normalizedErrorCode = String(errorCode ?? "")
        .trim()
        .toUpperCase();
    if (normalizedErrorCode === "ENOENT") return "404";
    if (normalizedErrorCode === "EACCES" || normalizedErrorCode === "EPERM") {
        return "403";
    }
    if (normalizedErrorCode === "ETIMEDOUT") return "504";
    if (
        ...
        try {
            ...
            if (url.pathname === "/invite") {
                ...
                const registrationGateway =
                    gatewayRegistry?.get("registration");
                if (
                    !registrationGateway ||
            ...
            if (
                ...
                const gatewayMessages = (
                    uiRegistry?.listAuthTypingMessages() ?? []
                )
                    .filter((message) => {
            ...
            if (
                ...
                if (!claims) return true;
                const sections = (
                    uiRegistry?.listSettingsSections() ?? []
                ).filter(
                    (section) =>
            ...
            if (
                url.pathname === "/api/v1/ui/app-routes" &&
                req.method === "GET"
            ) {
                const claims = ctx.requireAuth(req, res, "user");
                if (!claims) return true;
            ...
            if (relative === null) {
                if (!url.pathname.startsWith("/api/") && req.method === "GET") {
                    res.writeHead(302, { location: "/error?code=404" });
                    res.end();
                    return true;
                }
                return false;
            ...
            const errorCode = resolveUiRouteErrorCode(error);
            const statusCode = Number(errorCode);
            log?.("error", "Unhandled UI route failure.", {
                component: "api-ui",
                path: url.pathname,
                method: req.method ?? "GET",
                error: error instanceof Error ? error.message : String(error),
            ...
            if (req.method === "GET" && !url.pathname.startsWith("/api/")) {
                ...
                res.end(
                    "<!doctype html><html><body><h1>Unexpected error</h1><p>Unable to render the error page.</p></body></html>",
                );
                return true;
            }
            res.writeHead(statusCode, {
