...
async function canViewPost(
    ...
    if (author.visibility === 'friends') {
        const isFollower = await profileStore.isFollowing(requesterId, post.accountId);
    ...
    switch (post.visibility) {
        ...
        case 'private': return profileStore.isFollowing(requesterId, post.accountId);
        case 'friends': return profileStore.isFollowing(requesterId, post.accountId);
    ...
    return async (req: IncomingMessage, res: ServerResponse, url: URL): Promise<boolean> => {
        if (url.pathname === '/api/v1/posts' && req.method === 'POST') {
            ...
            if (!profile) {
                ...
                res.end(JSON.stringify({ error: { code: 'forbidden', message: 'Profile required to post' } }));
                return true;
            }
            if (profile.visibility === 'hidden') {
                res.writeHead(403, { 'content-type': 'application/json' });
                res.end(JSON.stringify({ error: { code: 'forbidden', message: 'Update your visibility to post' } }));
                return true;
            }
            ...
            if (!content) {
                ...
                res.end(JSON.stringify({ error: { code: 'bad_request', message: 'content is required' } }));
                return true;
            }
            const rawVisibility = String(body.visibility ?? 'community');
            if (!VALID_POST_VISIBILITY.has(rawVisibility as PostVisibility)) {
                res.writeHead(400, { 'content-type': 'application/json' });
                res.end(JSON.stringify({ error: { code: 'bad_request', message: `Invalid visibility: ${rawVisibility}` } }));
                return true;
            }
        ...
        if (deleteMatch && req.method === 'DELETE') {
            ...
            if (!post) {
                ...
                res.end(JSON.stringify({ error: { code: 'not_found', message: 'Post not found' } }));
                return true;
            }
            if (post.accountId !== claims.sub && claims.role !== 'admin' && claims.role !== 'moderator') {
                res.writeHead(403, { 'content-type': 'application/json' });
                res.end(JSON.stringify({ error: { code: 'forbidden', message: 'Cannot delete another user\'s post' } }));
                return true;
            }
        ...
        const userPostsMatch = url.pathname.match(/^\/api\/v1\/users\/([^/]+)\/posts$/);
        if (userPostsMatch && req.method === 'GET') {
            ...
            const allPosts = await profileStore.getPostsByAccount(author.accountId);
            const visible: Post[] = [];
            for (const post of allPosts) {
                if (await canViewPost(claims.sub, claims.role, post, author, profileStore)) {
                    visible.push(post);
                }
            }
            res.writeHead(200, { 'content-type': 'application/json' });
...
function postVisibilityRank(v: PostVisibility): number {
    const ranks: Record<PostVisibility, number> = { only_me: 0, private: 1, friends: 2, community: 3 };
    return ranks[v] ?? 0;
}
