...
export class LdapAuthGateway implements AuthGateway {
  ...
  async authenticate(token: string): Promise<AuthContext | null> {
    ...
    const isAdmin = (identity.groups ?? []).some((group) => this.adminGroups.has(group));
    const existing = await this.options.accountStore.findByExternalIdentity('ldap', identity.id);
