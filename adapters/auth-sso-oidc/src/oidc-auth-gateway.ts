...
export class OidcAuthGateway implements AuthGateway {
  ...
  async authenticate(token: string): Promise<AuthContext | null> {
    ...
    const isAdmin = (claims.roles ?? []).some((role) => this.adminRoles.has(role));
    const existing = await this.options.accountStore.findByExternalIdentity(this.providerName, claims.sub);
  ...
  async createLocalAdmin(): Promise<AuthContext> {
    throw new Error('OIDC SSO adapter cannot create local admins directly.');
  }
}
