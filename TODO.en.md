...
## Nextcloud Whiteboard module boundary migration

The installed `nextcloud-whiteboard` module still imports `/static/reuse/ui-ctx.js` and styles the protected `btn-cancel` class. Cognis cannot safely rewrite these independently versioned module files during installation because allowing either behavior would violate the external-module isolation boundary. Update `Cognis-Labs-HQ/cognis-module-nextcloud-whiteboard` to consume a deliberately exposed module UI capability surface and style only module-owned classes. Cognis now mounts the module’s API entrypoint in a restricted disabled context, so its explicitly opted-in configuration routes are available before enablement without activating feature routes, UI contributions, flows, or capabilities; the remaining two module-owned violations still require a compliant module release.

## Analytics module boundary migration

The published `analytics` module contains a dormant `api/invalid-access.js` source file that addresses another module namespace and a hard-coded `9px` chart-label font size. The external-module contract requires every installed source and stylesheet to pass boundary validation, including files that are not current entrypoints, because a later dynamic import could otherwise bypass activation checks. Update `Cognis-Labs-HQ/cognis-module-analytics` to remove the invalid route fixture from the distributable module and express chart-label sizing relative to the user’s selected font. Cognis continues to reject this release rather than weakening cross-module route isolation or font-preference enforcement.
