...
The installed `nextcloud-whiteboard` module still imports `/static/reuse/ui-ctx.js`, styles the protected `btn-cancel` class, and advertises component configuration without declaring a dedicated `entrypoints.disabledApi`. Cognis cannot safely rewrite these independently versioned module files during installation: allowing the direct core import or protected-class override would violate the external-module isolation boundary, while loading its normal API entrypoint while disabled would execute untrusted module runtime code before activation. Update `Cognis-Labs-HQ/cognis-module-nextcloud-whiteboard` to consume a deliberately exposed module UI capability surface, style only module-owned classes, and register its configuration routes from a restricted disabled API entrypoint. Until that module release is installed, Cognis now reports `module_config_unavailable` instead of a misleading 404 and boundary validation reports only the genuine cross-boundary violations rather than module-owned API URLs.
