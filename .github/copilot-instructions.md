...
- All page titles (keys ending in `page_title`), section headings (keys ending in `.title`), and navigation menu items (`ui.reuse.menu.*`) in English strings must use Title Case — every word capitalized. This rule applies to English (`en/strings.xml`) only; other languages follow their own capitalization conventions. A test in `src/ui/tests/title-case.test.js` enforces this for the English file.
