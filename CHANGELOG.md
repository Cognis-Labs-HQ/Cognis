...
- `DATA_ENCRYPTION_KEY` is now a required environment variable in production `docker-compose.yaml` (declared with the `${DATA_ENCRYPTION_KEY:?…}` pattern, mirroring `POSTGRES_PASSWORD`). `docker-compose.dev.yaml` defaults it to a clearly-marked dev-only insecure value so local development continues to work without explicit setup. ([TBD](https://github.com/le-firehawk/Cognis/commit/TBD))
