...
- **Study Gateway** (`src/gateways/study/`, version `1.0.0`). Thin gateway for study features; discovers and bootstraps adapters under `src/adapters/study/`. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- **Classes Adapter** (`src/adapters/study/classes/`, version `1.0.0`). Manages per-language teacher classes and teacher-request approval workflow. SQL tables: `study_classes`, `teacher_requests`, `teacher_assignments`. Routes under `/api/v1/study/*`. Page at `/classes` (teacher-only). ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- **Profile avatar role badges**. Admins see a 🔧 spanner badge at the bottom-left of their avatar; teachers see a 📖 book badge at the bottom-right. Implemented via `.profile-avatar-badge` CSS overlay on `.profile-avatar-wrap`. Replaces the text `profile-role-badge` pill in the profile hero. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- Inline role icon (🔧 for admin, 📖 for teacher) in followers / following / suggested contacts lists, replacing the text pill. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- **Visibility pill now reads "Visible to: {visibility}"** — uses the new `ui.app.profile.visible_to` key with `{visibility}` placeholder, translated in English, German, Japanese, and Indonesian. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- **Page composer `max: 'fill'`** — new sizing option that expands an element to fill remaining columns (or rows) from its placement position to the grid edge. Supported in `max`, `max: ['fill', n]`, `max: [n, 'fill']`, and `max: ['fill', 'fill']`. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- **Page composer half-width odd-column fill** — when a `halfWidth` element is placed and only one column remains to the right edge, the element expands by one column to eliminate the gap. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- My Classes menu item added to the user dropdown, visible to teachers and admins. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- New i18n keys under `module.study.classes.*` and `ui.reuse.menu.classes` translated for English, German, Japanese, and Indonesian. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- Post visibility badge now shows the translated post-visibility label (e.g. "Everyone", "Followers") rather than raw internal value. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
...
- **Social Gateway** (renamed from Profile Gateway, version `1.2.0`). The gateway is now thin: it discovers and bootstraps adapters under `src/adapters/social/<id>/` and exposes their capabilities via `ctx.capabilities`. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- **Profile Adapter** (`src/adapters/social/profile/`, version `1.0.0`). Owns the previous profile/post/social-graph/file/preferences code that lived in the profile gateway. Public HTTP endpoints (`/api/v1/profile`, `/api/v1/users/:handle/follow`, etc.) and capability names (`profile:createProfile`, `profile:setRoleByHandle`, `preferences:store`) are unchanged. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- **Messages Adapter** (`src/adapters/social/messages/`, version `1.0.0`). Provides private DM and group chatrooms with per-room AES-GCM encryption and at-rest re-wrapping using `DATA_ENCRYPTION_KEY`. Routes under `/api/v1/messages/*`. Notification dispatch is wired through the `notify:dispatch` capability with a new `messages` category (per-room mute supported). See `src/adapters/social/messages/docs/standard.en.md` for the full threat model. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- New `GET /api/v1/users/:handle/relationship` endpoint returning `{ following, followedBy, blocked, blockedBy, canMessage }` so the UI can derive relationship state from a single call. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- Messages page at `/messages` (and `/messages/:roomId`) built with `createPageComposer`. Registered with the SPA router and gated by a lightweight `GET /api/v1/messages/ping` probe so the navbar entry only appears when the messages adapter is loaded. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- Message ✉ icon button on non-self profiles, shown when `canMessage` is true; clicking it opens (or creates) the DM. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- New i18n keys under `module.social.messages.*` and `ui.reuse.nav.messages` / `ui.reuse.menu.messages` translated for English, German, Indonesian, and Japanese. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- Notify gateway now contributes `notify:gateway`, `notify:dispatch`, and `notify:registerCategory` capabilities so other adapters can hand off delivery without holding a direct gateway reference. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))

### Changed

- Profile post composer is now hidden on profiles other than the current user's own. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- Internal notification adapter now persists notifications to the application database (encrypted). Each user's notification payload (subject, body, category, sender, action URL) is encrypted at rest with a per-scope AES-256-GCM key derived via HKDF-SHA-256 from a server-side `DATA_ENCRYPTION_KEY` environment variable and a scope string of the form `user:notifications:<username>`. The general-purpose `getDataEncryptionKey`, `deriveScopedKey`, `encryptPayload`, and `decryptPayload` helpers live in `src/api/reuse/crypto.ts` so future subsystems (e.g. MFA secrets, media metadata) can reuse the same key-derivation primitives by passing their own scope string. The new `reuse/db-store.ts` provides `DbInternalNotificationStore` which replaces the in-memory store when a database executor is available. Falls back to in-memory storage if no executor is provided. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- `DATA_ENCRYPTION_KEY` is now a required environment variable in production `docker-compose.yaml` (declared with the `${DATA_ENCRYPTION_KEY:?…}` pattern, mirroring `POSTGRES_PASSWORD`). `docker-compose.dev.yaml` defaults it to a clearly-marked dev-only insecure value so local development continues to work without explicit setup. The internal notification adapter bootstrap now throws (refuses to start) when `DATA_ENCRYPTION_KEY` is unset under `NODE_ENV === "production"`. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- When `DATA_ENCRYPTION_KEY` is unset outside production, `deriveScopedKey` now uses a per-process random fallback (held only in memory) instead of a globally-known hardcoded default. This eliminates the risk of any attacker decrypting data from a misconfigured deployment using a value learned from the source tree, and makes the misconfiguration loud (data does not survive restart). ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- `NotifyAdapterBootstrapCtx` extended with optional `dbExecutor` and `dbType` fields so adapter bootstrap functions can access the application database when needed. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- `formatRelativeTime(epochMs, fallback)` added to `src/ui/reuse/timestamp.js`. Uses `Intl.RelativeTimeFormat` to produce locale-aware relative timestamps (e.g. "5 minutes ago", "2 hours ago") without custom translation strings. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- Page composer edit panel now includes a "↺ Reset Layout" button below the Discard/Done row. Clicking it clears all custom element placements for the current grid column count, reinitialises the layout to default positions, saves the reset state, and re-renders. The same button is available in sub-composer panels. This gives users a recovery path when elements are forced below their declared minimum size on narrow screens. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))

### Changed

- Notification list items now display as true multi-line rows: subject (bold), sender (accented), and content preview stack vertically as flex column. The relative timestamp ("X seconds/minutes/hours/days ago") is shown right-aligned outside the body column instead of as a fourth line inside it. CSS updated to add `display: flex; flex-direction: column; gap: 2px` on `.notification-item-body` and reposition `.notification-item-time` as a right-aligned sibling flex item. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- Internal notification adapter version bumped to `0.5.0` (`src/adapters/notify/internal/package.json`, `manifest.json`). ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- Notification bell button no longer renders a circle background on hover (or otherwise): the `.notification-button` `border-radius: 999px` and the `:hover { background: var(--surface-soft) }` rule were removed; the bell now sits flat in the topbar with no surrounding shape. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- Clicking the body of an arrival toast popup now also marks the underlying notification as read on the server (`PUT /api/v1/notifications/inbox/:id/read`), updates the cached `currentNotifications` entry, refreshes the badge count, and (if the panel is open) flips the corresponding list item from the unread to read style. The dismiss "×" button still only dismisses the toast without marking as read. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- Notification panel relative timestamps ("X seconds/minutes ago") now tick up live: each `.notification-item-time` carries its `createdAt` as `data-relative-time`, and a 1-second interval started on panel open (and torn down on close) re-renders every visible time span via `formatRelativeTime`. No additional network polling. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- Notification panel header now includes a 🗑 trash button (top-right) that clears every notification for the current user — the action prompts a danger-style `openPopup` confirmation, then issues `DELETE /api/v1/notifications/inbox` (new bulk-delete route) which calls a new `deleteAll(username)` method on the store interface (in-memory and `DbInternalNotificationStore` implementations both wipe DB rows for the user). ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- Notification panel now updates in real time while it is open: the inbox poll tick previously skipped any work whenever the panel was visible (silently dropping new arrivals from both the list and the popup toast); it now calls a new `refreshOpenPanel(i18n)` helper that re-fetches the inbox, detects arrivals, re-renders the list, and updates the badge. Popups remain suppressed while the panel is open (the user is already looking at the list). ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))

### Fixed

- `bootstrapAdapters` in `CoreNotificationGateway` was wrapping each adapter's `bootstrapNotifyAdapter()` call inside the same try-catch used to skip unreadable `package.json` entries, so a deliberate startup throw (e.g. missing `DATA_ENCRYPTION_KEY` in production) was swallowed and the process continued with the in-memory store instead of failing fast. The file/import error handling is now a separate inner try-catch that only covers filesystem and module-load errors; `bootstrapNotifyAdapter()` is called outside that guard so fatal bootstrap errors propagate to the caller. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- `createGatewayAdapterRoutes` in `bootstrap.ts` referenced `log` and `logMeta` inside the adapter-disable guard, but neither name is in scope in that function; the request would throw a `ReferenceError` before the intended 403 was sent, defeating the always-on protection. The guard now uses the sender's own `locked` field (already set by `registerAlwaysOnSender`) instead of comparing against the hard-coded adapter ID `"internal"`, and the out-of-scope log call has been removed. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e)): the notify gateway's `discoverSenders()` always invokes adapter factories as `factory(process.env)`, but the internal adapter's `createNotificationSender` treated arg-1 as a `storeOverride`. The result was `this.storeOverride = process.env`, and every `send()` call ran `process.env.add(envelope)` — a `TypeError` swallowed by `dispatch()`'s catch, dropping every notification. Notifications never reached the in-memory or DB store, so recipients saw nothing. The public factory now ignores its argument and resolves the store lazily from the module-level `activeStore` (which `bootstrapNotifyAdapter` upgrades to `DbInternalNotificationStore`, restoring cross-login persistence). Test code that needs to inject a store should call the new `createInternalNotificationSenderForTesting(store)` helper. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
...
- Docker container build failed with `npm ci` reporting `@cognis/adapter-notify-internal@0.2.0` missing from the lock file. Regenerated `package-lock.json` after the adapter version bump so the lock file is in sync with all workspace `package.json` manifests. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- Page composer elements with `max: 'full'` or `max: 'half'` did not auto-correct their stored column span when the grid column count changed (e.g. browser window resized or layout profile loaded from a different screen width). `computeViewPlacements` and `computeSubViewPlacements` now detect when a `fullWidth` element's stored width differs from `gridCols`, or when a `halfWidth` element's stored width differs from its target half-width, and trigger a repack. `repackPlacementsIntoColumns` now applies the semantically correct width for `fullWidth` and `halfWidth` elements using the element's `gridSize` definition, instead of always capping at `Math.min(orig.w, maxCols)`. ([766112e](https://github.com/le-firehawk/Cognis/commit/766112e))
- Ambiguous 1–2-letter variable names (`el`, `gs`, `ft`, `ro`, `gw`, `ts`, `li`, `iv`, `tz`, `to`, `tr`, `cx`, `cy`, `ch`, `rW`, `rH`, `db`) renamed throughout all scanned source files to their full descriptive forms. The `ambiguous-names.test.js` enforcement test extended from single-letter to also cover two-letter declarations, with an explicit allow-list (`id` plus positional shorthands `x`, `y`, `w`, `h`). ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))

### Added

- Notification gateway (`src/gateways/notify/manifest.json`) and internal notification adapter (`src/adapters/notify/internal/manifest.json`) marked `"required": true` so the gateway cannot be disabled via the admin API. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- `CoreNotificationGateway.disableSender()` now throws a `required_sender` error when the caller attempts to disable an always-on sender (such as the internal adapter). The adapter toggle route in `src/gateways/notify/bootstrap.ts` catches this and responds with HTTP 403 `required_sender`. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- Notification panel background replaced undefined `--card-bg` CSS variable with `--surface-2`, which is the same fully-opaque surface token used by the profile dropdown. The panel is now fully opaque across dark and light themes. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- Notification panel and profile menu are now mutually exclusive: opening the notification panel programmatically closes the profile menu, and a `MutationObserver` on `.profile-menu` closes the notification panel the moment the profile menu opens. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))

### Added

- Internal notification adapter (`src/adapters/notify/internal/`) that delivers all gateway-dispatched notifications to the in-app notification bell. The adapter is always-on by default and bypasses the user preference matrix. It stores up to 50 notifications per user in memory, exposes five inbox API routes (`GET /inbox`, `GET /inbox/count`, `PUT /inbox/read`, `PUT /inbox/:id/read`, `DELETE /inbox/:id`), and registers a navbar plugin that injects the notification bell button into the dashboard, polls for new notifications, and provides a dropdown panel for reading and dismissing notifications. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- `CoreNotificationGateway.bootstrapAdapters(adaptersRoot, ctx)` in `src/gateways/notify/gateway.ts`: a discovery pass that calls `bootstrapNotifyAdapter(ctx)` on any adapter module that exports it, giving adapters a generic hook to self-register routes, static directories, navbar plugins, and always-on sender status. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- `CoreNotificationGateway.registerAlwaysOnSender(senderId)` and `alwaysOnSenders` set: senders registered here are included in every `dispatch()` call regardless of the recipient's notification preference settings, so adapters can deliver to all users by default. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- `NotifyAdapterBootstrapCtx` interface in `src/gateways/notify/gateway.ts`: typed context passed to `bootstrapNotifyAdapter`, providing `registerRoute`, `registerNavbarPlugin`, `registerStaticDir`, a back-reference to the gateway, and an optional `log` function. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- Locale strings `ui.adapter.notify.internal.mark_all_read` and `ui.adapter.notify.internal.empty` added to all four language files (en, de, ja, id). ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- `alwaysOn?: boolean` field added to `NotificationSenderInfo` interface; `listSenders()` now marks always-on adapters so consumers can distinguish them. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- Notification arrival toast: the navbar plugin shows a bell-icon popup (`.arrival-toast`) in the top-right corner whenever new unread notifications arrive, stacking like system toasts. Clicking the popup opens the notification panel; the dismiss button or auto-dismiss after 6 s removes it. CSS lives in `notifications.css`. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))

### Changed

- Notification bell button removed from `src/ui/public/templates/dashboard-layout.html` and its CSS removed from `src/ui/styles/reuse/layout.css`. Both are now owned entirely by the internal notification adapter and rendered via its navbar plugin and `notifications.css`. Disabling or removing the adapter makes the bell disappear from the UI. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- Navbar plugin polling changed from a fixed 30 s interval to an adaptive schedule: 10 s when the tab is visible, 30 s when hidden. The initial poll now fetches the full notification list (rather than just the count) to seed the seen-ID tracking, preventing stale notifications from triggering arrival popups on load. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- Notification preferences matrix (`src/ui/app/settings/notification-prefs.js`) now renders always-on senders (flagged by `alwaysOn: true` in the providers list) as pre-checked and disabled. The `ui.adapter.notify.internal.always_on` tooltip string is removed from all four language files. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- Notification gateway registered with `required: true` in `gatewayRegistry.register()` so the generic gateway route handler blocks disable attempts at the API layer, consistent with how the logging, database, and authentication gateways are protected. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- Internal notification adapter disable prevention moved from a `disableSender()` error-throw approach to a direct hardcoded guard (`adapterId === "internal" && action === "disable"`) in the adapter toggle route handler, consistent with how the local authentication adapter is protected. The `required_sender` throw is removed from `CoreNotificationGateway.disableSender()`. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))

- PWA support so Cognis can be installed as an app from Chrome / Chromium / Edge ("Install Cognis…" / "Add to Home screen") and from Firefox / Firefox for Android. New `src/ui/public/manifest.webmanifest` declares the app name, scope (`/`), `start_url` (`/dashboard`), `display: standalone` (with `display_override: ["standalone","minimal-ui","browser"]`), theme/background colours, dashboard + settings shortcuts, and four PNG icons (192/512 in both `purpose: "any"` and `purpose: "maskable"`) generated from the existing brand asset. New `src/ui/public/sw.js` service worker precaches a small app shell, serves navigation requests network-first with an offline fallback to the cached `/dashboard` shell, serves `/static/*` and the manifest stale-while-revalidate, and never intercepts `/api/*` so authentication and live data always go to the network. New `src/ui/reuse/pwa.js` registers the worker at root scope, captures `beforeinstallprompt` for a future install button, and is hooked into both the dashboard layout (post-auth pages) and the auth shell + verify-email page (pre-auth pages) so installation is offered from every entry point. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- Root-scope routes for `/manifest.webmanifest` and `/sw.js` in `src/api/routes/ui/index.ts`. The service-worker response includes `Service-Worker-Allowed: /` so the worker may control the entire site even though it is delivered from `/sw.js`. The manifest is served with the standard `application/manifest+json` MIME type. Both bypass the HTML page CSP wrapper since they are not HTML, but still set `X-Content-Type-Options: nosniff`. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- New PWA icon assets generated from `cognis-icon.png`: `cognis-icon-192.png`, `cognis-icon-512.png`, `cognis-icon-maskable-192.png`, `cognis-icon-maskable-512.png`, and `cognis-apple-touch-180.png` (iOS home-screen). Maskable variants pad the brand mark inside the safe zone over the dark slate background colour so adaptive-icon themes on Android do not crop the artwork. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- Tests in `src/api/tests/ui/ui-routes.test.ts` covering both the manifest endpoint (MIME type, `name`, `start_url`, `scope`, `display`, presence of 192/512 icons and a maskable variant) and the service-worker endpoint (root-scope header, JS content type, install/fetch handler presence). ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- All HTML pages under `src/ui/public/pages/` now declare PWA metadata: `<link rel="manifest">`, `<meta name="theme-color" content="#0f172a">`, `<meta name="application-name">`, `apple-mobile-web-app-*` and `mobile-web-app-capable` meta tags, an Apple touch icon link, and `viewport-fit=cover` so the app renders edge-to-edge on devices with notches when launched standalone. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- Page CSP (`setPageSecurityHeaders` in `src/api/auth/guard.ts`) now includes explicit `worker-src 'self'` and `manifest-src 'self'` directives. These were previously covered implicitly by `default-src 'self'`, but explicit directives make the policy auditable for the new service-worker and manifest fetches. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- Client-side SPA router (`src/ui/reuse/app-router.js`): intercepts clicks on internal navigation links and uses `history.pushState()` to mount new page content in place via each page's `mount()` function — no full browser reload. The router creates an `AbortController` per navigation and passes its signal to `mount()` so page-level `window` event listeners (e.g. `popstate` in docs, `beforeunload` in administration) are automatically cleaned up when the user navigates away. A `signal.aborted` guard after each dynamic import prevents mounting a page whose navigation was superseded mid-load. ([c9fdc2e](https://github.com/le-firehawk/Cognis/commit/c9fdc2e))
- All eight dashboard pages (`dashboard`, `settings`, `modules`, `users`, `profile`, `administration`, `docs`, `license`) now export `async function mount(root, { signal } = {})` so the router can populate them in-place. Each page calls `await mount(document.querySelector('#app'))` at module level for direct URL access and hard loads, guarded by `if (!globalThis.__spaRouter)` so the call is skipped when the router imports the module (see the double-mount fix in Fixed). ([c9fdc2e](https://github.com/le-firehawk/Cognis/commit/c9fdc2e))
...
- Notification gateway (`src/gateways/notify/manifest.json`) and internal notification adapter (`src/adapters/notify/internal/manifest.json`) marked `"required": true` so the gateway cannot be disabled via the admin API. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- Internal notification adapter disable prevention aligned with codebase convention: the adapter toggle route handler in `src/gateways/notify/bootstrap.ts` now guards `adapterId === "internal" && action === "disable"` upfront and returns HTTP 403 `locked_adapter` before calling any gateway method, matching the pattern used by the local auth adapter. The previous approach (throwing `required_sender` from `CoreNotificationGateway.disableSender()` and catching it in the route) is removed. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- `NotificationSenderInfo` now includes a `locked?: boolean` field; `CoreNotificationGateway.listSenders()` sets it to `true` for all always-on senders (including "internal"). The administration page reads `adapter.locked` to disable the toggle slider in the UI, matching the pattern used by the local authentication adapter in `AuthGateway.listAdapters()`. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- Notification arrival-toast container is now positioned dynamically below `.global-navrow` via `getBoundingClientRect()` instead of a hardcoded `top: 72px` CSS value, so toasts always appear beneath the full site header. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- Notification item and toast now show the sender name (from `NotificationEnvelope.senderName`); when no sender is provided the label falls back to a localised "System" string. `senderName` is stored on `InternalNotification` and returned by the inbox route. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- Notification item timestamp replaced the stale `formatRelativeTime()` with `formatDateTime()` from `src/ui/reuse/timestamp.js`, producing a locale-aware absolute date/time that routes through the shared timestamp utility. The `.notification-item-time` element gains `overflow: hidden; text-overflow: ellipsis` to prevent x-overflow in the panel. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- `NotificationEnvelope` gains an optional `actionUrl?: string` field. When set on an internal notification, clicking the panel item or arrival toast navigates the SPA router to that URL (or opens it in a new tab for external URLs) instead of just marking the notification as read. Items with an `actionUrl` display a `›` chevron indicator and carry the `notification-item--linked` class. Arrival toasts with an `actionUrl` navigate directly on click rather than opening the notification panel. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- Notification panel background replaced undefined `--card-bg` CSS variable with `--surface-2`, which is the same fully-opaque surface token used by the profile dropdown. The panel is now fully opaque across dark and light themes. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- Notification panel and profile menu are now mutually exclusive: opening the notification panel programmatically closes the profile menu, and a `MutationObserver` on `.profile-menu` closes the notification panel the moment the profile menu opens. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- `CoreNotificationGateway.discoverSenders()` and `bootstrapAdapters()` previously imported each adapter with a `?t=${Date.now()}` cache-busting suffix, causing two separate module instances to be created per adapter. For the internal notification adapter this meant the `InternalNotificationSender` wrote to one `InternalNotificationStore` instance while the inbox routes read from a different one, so dispatched notifications never appeared in the inbox or triggered arrival toasts. Both methods now import each adapter with a stable URL so the ESM module cache is shared, ensuring sender and routes operate on the same store. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
...
- Module bootstrap: `readdir()` in `src/api/main.ts` was scanning all entries in `src/modules/` — including files (`README.md`, `gateway.ts`, `package.json`) and non-module subdirectories (`docs/`, `routes/`) — and emitting an `error` log for each one without a `manifest.json`. Changed to `readdir({ withFileTypes: true })` with an `.isDirectory()` filter (matching the pattern used by `GatewayService.bootstrap()`), and silently skip directories that have no `manifest.json` (ENOENT); only actual parse/read failures are still logged at error level. ([d32903f](https://github.com/le-firehawk/Cognis/commit/d32903f))
- Page composer: stale `ResizeObserver` from a previous SPA navigation now short-circuits immediately when its `contentGrid` is no longer in the document (`document.contains(contentGrid)` guard in the observer callback and at the top of `renderGridComposer()`). Without this guard, the old closure would call `document.getElementById("composer-elements-panel")?.remove()` and `syncEditToggle()` on the live DOM of the newly-mounted page, hijacking the edit-toggle button and eventually calling `saveLayout()` with the wrong `preferenceKey` and `gridCols` — corrupting the per-viewport layout profiles. ([67084a8](https://github.com/le-firehawk/Cognis/commit/67084a8))
- SPA router — double-mount on overlapping uncached navigations: `globalThis.__spaRouter` was a plain boolean cleared in the `finally` block of `route.load()`. If two uncached navigations overlapped, the first import to finish reset the flag to `false` while the second module was still evaluating — causing that module to run its direct-load `await mount(...)` path, then the router also called `mod.mount()`. Fixed by adding a reference-count (`globalThis.__spaRouterCount`): the flag is only cleared when the count drops back to zero, ensuring it stays `true` for the entire duration of any concurrent imports. Because JavaScript is single-threaded the increment executes atomically before any `await`, so there is no interleaving race between concurrent `loadRoute()` calls. Page modules require no change. ([348b371](https://github.com/le-firehawk/Cognis/commit/348b371))
- Profile page — stale mount replacing live DOM after navigation: `mount()` made many sequential awaited network calls (profile, followers/following/posts, avatar/banner blobs, banner-height preference) with no `signal.aborted` checks between them. If the user navigated away while those fetches were in flight, the router aborted the mount's signal but `mount()` continued to completion, calling `composer.init()` and replacing the DOM of the newly-loaded page. Added `isAborted()` helper and guards after the profile load, after the `Promise.all` for social data, and after the image/preference loads — before the composer writes to the DOM. ([348b371](https://github.com/le-firehawk/Cognis/commit/348b371))
- SPA router — page-specific CSS not loading on navigation: page-specific stylesheets (`settings.css`, `profile.css`, `char-counter.css`, `license.css`) were only present via `<link>` in each page's HTML file. On direct browser load this worked, but SPA navigation only loaded the JavaScript module, leaving the target page's CSS absent. The router now maintains a `stylesheets` list per route (mirroring each page's HTML) and uses a new `src/ui/reuse/page-styles.js` module to inject any missing `<link>` elements idempotently — injection runs in parallel with module loading so there is no added latency. ([fa21d56](https://github.com/le-firehawk/Cognis/commit/fa21d56))
- Settings — font and timezone pickers always showing defaults: `initFontPrefs` and `initDateTimePrefs` both destructure an `existingPrefs` parameter, but the `settings/index.js` call sites were passing `committedPrefs` as the key name (a holdover from renaming `existingPrefs` to `committedPrefs` at the outer scope). JavaScript destructuring with a mismatched key left `existingPrefs` as `undefined` inside both functions, so the font picker always initialized to `Orbitron` (DEFAULT_FONT) with the Reset button disabled, and the timezone selector always showed "auto" — regardless of what was saved on the server. Fixed by passing `existingPrefs: committedPrefs` at both call sites. ([7fd0a3c](https://github.com/le-firehawk/Cognis/commit/7fd0a3c))
- Settings — saving from Language/Notifications/DateTime section silently cleared the saved font: when a user saved without having visited the Appearance sub-page in the current mount, `fontPrefs` was `null`. The `onSave` handler fell back to `appFont: undefined` and `appFontSize: undefined`. JSON.stringify omits `undefined` keys, so the PUT request sent to the API contained no font fields — causing a server-side replace to clear those values. `applyUiPreferences` then applied an undefined font, reverting the page to the default typeface. `onSave` now falls back to `committedPrefs?.appFont` / `committedPrefs?.appFontSize` when `fontPrefs` is absent, preserving the last saved values. ([7fd0a3c](https://github.com/le-firehawk/Cognis/commit/7fd0a3c))
- Settings dirty tracker: after a successful save, the "saved" baseline in each preference controller (`initFontPrefs`, `initThemePrefs`, `initDateTimePrefs`, `initLanguagePrefs`, `initNotificationPrefs`) was never advanced past the value that was loaded at page mount. On the second edit cycle the comparison was against a stale reference, causing changes that happened to equal the original value to appear clean, and changes that matched the just-saved value to incorrectly appear dirty. Each controller now exposes a `commit()` method and `onSave` calls it on all controllers after the API writes succeed. ([5b900c2](https://github.com/le-firehawk/Cognis/commit/5b900c2))
- Profile page: navigating to a profile URL that does not exist (404) no longer throws an uncaught `Error('profile_not_found')` through the router. `mount()` now renders the not-found message and returns cleanly. The router also wraps `mount()` calls in a try/catch so any future mount-time errors are logged rather than surfacing as unhandled rejections. ([5b900c2](https://github.com/le-firehawk/Cognis/commit/5b900c2))
- Profile page: navigating to `/profile` (own-profile link, no handle in the path) produced a double-slash API request (`/api/v1/users//profile`) and a 404, because `urlHandle` was empty and `isOwnProfile` resolved to `false`. An empty `urlHandle` is now treated as own-profile, routing through `loadOwnProfile()` instead. `bannerMenuCloseHandler` declaration was also moved to the module-level variable block (lines 15-29) so the cleanup pattern in `mount()` is self-evidently correct. ([e331866](https://github.com/le-firehawk/Cognis/commit/e331866))
- Settings: language-change detection extracted into `hasLanguagePriorityChanged(prev, next)` helper so the comparison is named and reusable. ([e331866](https://github.com/le-firehawk/Cognis/commit/e331866))
- Settings page: `existingPrefs` (the preferences object loaded from the server at mount time) was declared `const` and never updated after a save. Because the page-composer unmounts and remounts each sub-composer section when the user switches between settings sub-pages, `onRender` fires each time — reinitializing `fontPrefs` and `datetimePrefs` from the stale `existingPrefs`. After a Reset+Save (or any save followed by sub-page navigation), the picker would reinitialize from the old server values, making the saved change appear to have been lost. `existingPrefs` is now `let` and is updated to the newly saved values immediately after `savePrefs()` succeeds, so any subsequent `onRender` calls initialize from fresh data. ([6ace3a5](https://github.com/le-firehawk/Cognis/commit/6ace3a5))
- SPA router: on the first navigation to a page that has not yet been imported, the browser evaluates the module and its top-level `await mount(document.querySelector('#app'))` ran immediately — before the router could call `mod.mount(_root, { signal })` — producing a double mount with duplicate API calls, duplicate event handlers, and for `/users?action=invite` a double invite flow trigger. The router now sets `globalThis.__spaRouter = true` inside a try/finally block around `route.load()` and clears it on completion. All eight dashboard page modules guard their top-level `await mount(...)` with `if (!globalThis.__spaRouter)` so it only executes on direct URL loads, never when imported by the router. ([489f508](https://github.com/le-firehawk/Cognis/commit/489f508))
- Docs page: the delegated `root.addEventListener("click", ...)` that handles in-document link clicks was registered without a cleanup mechanism — no `{ signal }` and no explicit `removeEventListener`. Every SPA navigation back to `/docs` added another handler to the persistent `#app` element, causing `showDoc` to fire multiple times per click and accumulating duplicate `pushState` history entries. The listener now passes `{ signal }` so it is automatically removed when the router aborts the mount controller on navigation away. ([489f508](https://github.com/le-firehawk/Cognis/commit/489f508))
- Light mode — email badges: `email-badge-primary` and `email-badge-verified` now render as tinted, dark-text pills in light mode instead of inheriting the dark-mode accent background with hardcoded white text; `email-badge-unverified` now uses a visible slate tint rather than the near-white `--control-bg` that made it invisible against the light page background. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
- Light mode — toggle slider: the off-state slider track now uses a light slate gradient (`#cbd5e1 → #94a3b8`) in light mode instead of the dark blue-grey values that look out of place on a light background; the knob shadow is softened to match. ([bf1109e](https://github.com/le-firehawk/Cognis/commit/bf1109e))
...
- Administration: added a new Logs page in Administration with a live app-log stream and server-side severity/keyword filtering via the logging gateway (`GET /api/v1/logging/stream`). ([e3f2dde](https://github.com/le-firehawk/Cognis/commit/e3f2dde))
...
- Logging consistency: logging call syntax is now normalized around a local `log` callable style (`log(...)` / `log?.(...)`) across runtime token and DB/account logging paths, and AI instructions now require a single logging invocation style in new code. ([465212d](https://github.com/le-firehawk/Cognis/commit/465212d))
