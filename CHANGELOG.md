...
- Notification preferences matrix (`src/ui/app/settings/notification-prefs.js`) now renders always-on senders (flagged by `alwaysOn: true` in the providers list) as pre-checked and disabled with an explanatory tooltip, so users can see that in-app notifications are always delivered without being able to uncheck them. ([TBD](https://github.com/le-firehawk/Cognis/commit/TBD))
