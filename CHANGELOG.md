...
- **Social Gateway** (renamed from Profile Gateway, version `1.2.0`). The gateway is now thin: it discovers and bootstraps adapters under `src/adapters/social/<id>/` and exposes their capabilities via `ctx.capabilities`. ([TBD](https://github.com/le-firehawk/Cognis/commit/TBD))
- **Profile Adapter** (`src/adapters/social/profile/`, version `1.0.0`). Owns the previous profile/post/social-graph/file/preferences code that lived in the profile gateway. Public HTTP endpoints (`/api/v1/profile`, `/api/v1/users/:handle/follow`, etc.) and capability names (`profile:createProfile`, `profile:setRoleByHandle`, `preferences:store`) are unchanged. ([TBD](https://github.com/le-firehawk/Cognis/commit/TBD))
- **Messages Adapter** (`src/adapters/social/messages/`, version `1.0.0`). Provides private DM and group chatrooms with per-room AES-GCM encryption and at-rest re-wrapping using `DATA_ENCRYPTION_KEY`. Routes under `/api/v1/messages/*`. Notification dispatch is wired through the `notify:dispatch` capability with a new `messages` category (per-room mute supported). See `src/adapters/social/messages/docs/standard.en.md` for the full threat model. ([TBD](https://github.com/le-firehawk/Cognis/commit/TBD))
- New `GET /api/v1/users/:handle/relationship` endpoint returning `{ following, followedBy, blocked, blockedBy, canMessage }` so the UI can derive relationship state from a single call. ([TBD](https://github.com/le-firehawk/Cognis/commit/TBD))
- Messages page at `/messages` (and `/messages/:roomId`) built with `createPageComposer`. Registered with the SPA router and gated by a lightweight `GET /api/v1/messages/ping` probe so the navbar entry only appears when the messages adapter is loaded. ([TBD](https://github.com/le-firehawk/Cognis/commit/TBD))
- Message ✉ icon button on non-self profiles, shown when `canMessage` is true; clicking it opens (or creates) the DM. ([TBD](https://github.com/le-firehawk/Cognis/commit/TBD))
- New i18n keys under `module.social.messages.*` and `ui.reuse.nav.messages` / `ui.reuse.menu.messages` translated for English, German, Indonesian, and Japanese. ([TBD](https://github.com/le-firehawk/Cognis/commit/TBD))
- Notify gateway now contributes `notify:gateway`, `notify:dispatch`, and `notify:registerCategory` capabilities so other adapters can hand off delivery without holding a direct gateway reference. ([TBD](https://github.com/le-firehawk/Cognis/commit/TBD))

### Changed

- Profile post composer is now hidden on profiles other than the current user's own. ([TBD](https://github.com/le-firehawk/Cognis/commit/TBD))
...
- `DATA_ENCRYPTION_KEY` is now a required environment variable in production `docker-compose.yaml` (declared with the `${DATA_ENCRYPTION_KEY:?…}` pattern, mirroring `POSTGRES_PASSWORD`). `docker-compose.dev.yaml` defaults it to a clearly-marked dev-only insecure value so local development continues to work without explicit setup. The internal notification adapter bootstrap now throws (refuses to start) when `DATA_ENCRYPTION_KEY` is unset under `NODE_ENV === "production"`. ([TBD](https://github.com/le-firehawk/Cognis/commit/TBD))
- When `DATA_ENCRYPTION_KEY` is unset outside production, `deriveScopedKey` now uses a per-process random fallback (held only in memory) instead of a globally-known hardcoded default. This eliminates the risk of any attacker decrypting data from a misconfigured deployment using a value learned from the source tree, and makes the misconfiguration loud (data does not survive restart). ([TBD](https://github.com/le-firehawk/Cognis/commit/TBD))
...
- Docker container build failed with `npm ci` reporting `@cognis/adapter-notify-internal@0.2.0` missing from the lock file. Regenerated `package-lock.json` after the adapter version bump so the lock file is in sync with all workspace `package.json` manifests. ([TBD](https://github.com/le-firehawk/Cognis/commit/TBD))
...
- Ambiguous 1–2-letter variable names (`el`, `gs`, `ft`, `ro`, `gw`, `ts`, `li`, `iv`, `tz`, `to`, `tr`, `cx`, `cy`, `ch`, `rW`, `rH`, `db`) renamed throughout all scanned source files to their full descriptive forms. The `ambiguous-names.test.js` enforcement test extended from single-letter to also cover two-letter declarations, with an explicit allow-list (`id` plus positional shorthands `x`, `y`, `w`, `h`). ([TBD](https://github.com/le-firehawk/Cognis/commit/TBD))

### Added

- Notification gateway (`src/gateways/notify/manifest.json`) and internal notification adapter (`src/adapters/notify/internal/manifest.json`) marked `"required": true` so the gateway cannot be disabled via the admin API. ([TBD](https://github.com/le-firehawk/Cognis/commit/TBD))
- `CoreNotificationGateway.disableSender()` now throws a `required_sender` error when the caller attempts to disable an always-on sender (such as the internal adapter). The adapter toggle route in `src/gateways/notify/bootstrap.ts` catches this and responds with HTTP 403 `required_sender`. ([TBD](https://github.com/le-firehawk/Cognis/commit/TBD))
- Notification panel background replaced undefined `--card-bg` CSS variable with `--surface-2`, which is the same fully-opaque surface token used by the profile dropdown. The panel is now fully opaque across dark and light themes. ([TBD](https://github.com/le-firehawk/Cognis/commit/TBD))
- Notification panel and profile menu are now mutually exclusive: opening the notification panel programmatically closes the profile menu, and a `MutationObserver` on `.profile-menu` closes the notification panel the moment the profile menu opens. ([TBD](https://github.com/le-firehawk/Cognis/commit/TBD))

### Added

- Internal notification adapter (`src/adapters/notify/internal/`) that delivers all gateway-dispatched notifications to the in-app notification bell. The adapter is always-on by default and bypasses the user preference matrix. It stores up to 50 notifications per user in memory, exposes five inbox API routes (`GET /inbox`, `GET /inbox/count`, `PUT /inbox/read`, `PUT /inbox/:id/read`, `DELETE /inbox/:id`), and registers a navbar plugin that injects the notification bell button into the dashboard, polls for new notifications, and provides a dropdown panel for reading and dismissing notifications. ([TBD](https://github.com/le-firehawk/Cognis/commit/TBD))
- `CoreNotificationGateway.bootstrapAdapters(adaptersRoot, ctx)` in `src/gateways/notify/gateway.ts`: a discovery pass that calls `bootstrapNotifyAdapter(ctx)` on any adapter module that exports it, giving adapters a generic hook to self-register routes, static directories, navbar plugins, and always-on sender status. ([TBD](https://github.com/le-firehawk/Cognis/commit/TBD))
- `CoreNotificationGateway.registerAlwaysOnSender(senderId)` and `alwaysOnSenders` set: senders registered here are included in every `dispatch()` call regardless of the recipient's notification preference settings, so adapters can deliver to all users by default. ([TBD](https://github.com/le-firehawk/Cognis/commit/TBD))
- `NotifyAdapterBootstrapCtx` interface in `src/gateways/notify/gateway.ts`: typed context passed to `bootstrapNotifyAdapter`, providing `registerRoute`, `registerNavbarPlugin`, `registerStaticDir`, a back-reference to the gateway, and an optional `log` function. ([TBD](https://github.com/le-firehawk/Cognis/commit/TBD))
- Locale strings `ui.adapter.notify.internal.mark_all_read` and `ui.adapter.notify.internal.empty` added to all four language files (en, de, ja, id). ([TBD](https://github.com/le-firehawk/Cognis/commit/TBD))
- `alwaysOn?: boolean` field added to `NotificationSenderInfo` interface; `listSenders()` now marks always-on adapters so consumers can distinguish them. ([TBD](https://github.com/le-firehawk/Cognis/commit/TBD))
- Notification arrival toast: the navbar plugin shows a bell-icon popup (`.arrival-toast`) in the top-right corner whenever new unread notifications arrive, stacking like system toasts. Clicking the popup opens the notification panel; the dismiss button or auto-dismiss after 6 s removes it. CSS lives in `notifications.css`. ([TBD](https://github.com/le-firehawk/Cognis/commit/TBD))

### Changed

- Notification bell button removed from `src/ui/public/templates/dashboard-layout.html` and its CSS removed from `src/ui/styles/reuse/layout.css`. Both are now owned entirely by the internal notification adapter and rendered via its navbar plugin and `notifications.css`. Disabling or removing the adapter makes the bell disappear from the UI. ([TBD](https://github.com/le-firehawk/Cognis/commit/TBD))
- Navbar plugin polling changed from a fixed 30 s interval to an adaptive schedule: 10 s when the tab is visible, 30 s when hidden. The initial poll now fetches the full notification list (rather than just the count) to seed the seen-ID tracking, preventing stale notifications from triggering arrival popups on load. ([TBD](https://github.com/le-firehawk/Cognis/commit/TBD))
- Notification preferences matrix (`src/ui/app/settings/notification-prefs.js`) now renders always-on senders (flagged by `alwaysOn: true` in the providers list) as pre-checked and disabled. The `ui.adapter.notify.internal.always_on` tooltip string is removed from all four language files. ([TBD](https://github.com/le-firehawk/Cognis/commit/TBD))
- Notification gateway registered with `required: true` in `gatewayRegistry.register()` so the generic gateway route handler blocks disable attempts at the API layer, consistent with how the logging, database, and authentication gateways are protected. ([TBD](https://github.com/le-firehawk/Cognis/commit/TBD))
- Internal notification adapter disable prevention moved from a `disableSender()` error-throw approach to a direct hardcoded guard (`adapterId === "internal" && action === "disable"`) in the adapter toggle route handler, consistent with how the local authentication adapter is protected. The `required_sender` throw is removed from `CoreNotificationGateway.disableSender()`. ([TBD](https://github.com/le-firehawk/Cognis/commit/TBD))
...
- Notification gateway (`src/gateways/notify/manifest.json`) and internal notification adapter (`src/adapters/notify/internal/manifest.json`) marked `"required": true` so the gateway cannot be disabled via the admin API. ([TBD](https://github.com/le-firehawk/Cognis/commit/TBD))
- Internal notification adapter disable prevention aligned with codebase convention: the adapter toggle route handler in `src/gateways/notify/bootstrap.ts` now guards `adapterId === "internal" && action === "disable"` upfront and returns HTTP 403 `locked_adapter` before calling any gateway method, matching the pattern used by the local auth adapter. The previous approach (throwing `required_sender` from `CoreNotificationGateway.disableSender()` and catching it in the route) is removed. ([TBD](https://github.com/le-firehawk/Cognis/commit/TBD))
- `NotificationSenderInfo` now includes a `locked?: boolean` field; `CoreNotificationGateway.listSenders()` sets it to `true` for all always-on senders (including "internal"). The administration page reads `adapter.locked` to disable the toggle slider in the UI, matching the pattern used by the local authentication adapter in `AuthGateway.listAdapters()`. ([TBD](https://github.com/le-firehawk/Cognis/commit/TBD))
- Notification arrival-toast container is now positioned dynamically below `.global-navrow` via `getBoundingClientRect()` instead of a hardcoded `top: 72px` CSS value, so toasts always appear beneath the full site header. ([TBD](https://github.com/le-firehawk/Cognis/commit/TBD))
- Notification item and toast now show the sender name (from `NotificationEnvelope.senderName`); when no sender is provided the label falls back to a localised "System" string. `senderName` is stored on `InternalNotification` and returned by the inbox route. ([TBD](https://github.com/le-firehawk/Cognis/commit/TBD))
- Notification item timestamp replaced the stale `formatRelativeTime()` with `formatDateTime()` from `src/ui/reuse/timestamp.js`, producing a locale-aware absolute date/time that routes through the shared timestamp utility. The `.notification-item-time` element gains `overflow: hidden; text-overflow: ellipsis` to prevent x-overflow in the panel. ([TBD](https://github.com/le-firehawk/Cognis/commit/TBD))
- `NotificationEnvelope` gains an optional `actionUrl?: string` field. When set on an internal notification, clicking the panel item or arrival toast navigates the SPA router to that URL (or opens it in a new tab for external URLs) instead of just marking the notification as read. Items with an `actionUrl` display a `›` chevron indicator and carry the `notification-item--linked` class. Arrival toasts with an `actionUrl` navigate directly on click rather than opening the notification panel. ([TBD](https://github.com/le-firehawk/Cognis/commit/TBD))
- Notification panel background replaced undefined `--card-bg` CSS variable with `--surface-2`, which is the same fully-opaque surface token used by the profile dropdown. The panel is now fully opaque across dark and light themes. ([TBD](https://github.com/le-firehawk/Cognis/commit/TBD))
- Notification panel and profile menu are now mutually exclusive: opening the notification panel programmatically closes the profile menu, and a `MutationObserver` on `.profile-menu` closes the notification panel the moment the profile menu opens. ([TBD](https://github.com/le-firehawk/Cognis/commit/TBD))
- `CoreNotificationGateway.discoverSenders()` and `bootstrapAdapters()` previously imported each adapter with a `?t=${Date.now()}` cache-busting suffix, causing two separate module instances to be created per adapter. For the internal notification adapter this meant the `InternalNotificationSender` wrote to one `InternalNotificationStore` instance while the inbox routes read from a different one, so dispatched notifications never appeared in the inbox or triggered arrival toasts. Both methods now import each adapter with a stable URL so the ESM module cache is shared, ensuring sender and routes operate on the same store. ([TBD](https://github.com/le-firehawk/Cognis/commit/TBD))
...
- Light mode — toggle slider: the off-state slider track now uses a light slate gradient (`#cbd5e1 → #94a3b8`) in light mode instead of the dark blue-grey values that look out of place on a light background; the knob shadow is softened to match. ([TBD](https://github.com/le-firehawk/Cognis/commit/TBD))
