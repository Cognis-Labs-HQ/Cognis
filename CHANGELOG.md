...
- `aside.toolbar` width cap: the sidebar toolbar column is now limited to `min(max-content, 40vw)` on desktop so it cannot exceed 40 % of the viewport; existing `overflow-wrap: anywhere` on the element ensures long words wrap rather than overflow. ([2bf8516](https://github.com/le-firehawk/Cognis/commit/2bf8516))
