...
- Responsive follow-up: global navbar compact mode now triggers only when primary links actually overflow available nav space; side-menu overlays are darker/less transparent, page-composer compact card wrapping is driven by effective grid density (not only viewport breakpoint), edit mode keeps scaled compact proportions, and page content no longer exposes horizontal overflow from these responsive overlays. ([8fd5929](https://github.com/le-firehawk/Cognis/commit/8fd5929))
