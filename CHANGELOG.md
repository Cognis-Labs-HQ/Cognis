...
- All eight dashboard pages (`dashboard`, `settings`, `modules`, `users`, `profile`, `administration`, `docs`, `license`) now export `async function mount(root, { signal } = {})` so the router can populate them in-place. Each page still calls `await mount(document.querySelector('#app'))` at module level for direct URL access and hard loads. ([c9fdc2e](https://github.com/le-firehawk/Cognis/commit/c9fdc2e))
