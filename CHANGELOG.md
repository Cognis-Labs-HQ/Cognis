...
- Page composer follow-up: Elements edit panel now clamps horizontally to the workspace bounds (not only viewport bounds) so it no longer drifts outside the main page area, and grid-profile selection now re-applies the correct saved profile when grid columns change to keep desktop/mobile layouts isolated. ([pending](https://github.com/le-firehawk/Cognis/commit/pending))
