...
- Navbar/account and composer behavior polish: `account-cluster` now remains pinned to the far-right navbar column in both regular and compact nav states; toolbar collapse now defaults to collapsed (`▸`/`◂` icons), re-collapses after toolbar option selection, and entering page-composer edit mode now starts from the currently adjusted responsive grid instead of reverting to wider stored placement positions. ([3658168](https://github.com/le-firehawk/Cognis/commit/3658168))
